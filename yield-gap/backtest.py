#!/usr/bin/env python3
"""
Yield Gap Screen Backtest

Annual rebalancing (January), equal weight, top 30 by highest earnings yield gap.
Fetches data via configurable provider, caches in DuckDB, runs locally.

Signal: Stocks offering an earnings yield significantly above the regional
        risk-free rate (at least SPREAD_ABOVE_RFR percentage points).
        This is the stock-level proxy for the equity risk premium (yield gap)
        being unusually wide.

Filters:
  - Earnings yield > max(EARNINGS_YIELD_MIN, rfr + SPREAD_ABOVE_RFR)
  - Earnings yield < 0.50 (cap at 50% to exclude data errors / distressed)
  - ROE > ROE_MIN (quality filter, avoid value traps)
  - D/E ratio < DE_MAX (not dangerously leveraged)
  - Market cap > exchange-specific threshold

Portfolio: Top 30 by highest earnings yield, equal weight.
           Cash if < 10 stocks qualify.
Rebalancing: Annual (January), 2000-2025.

Academic reference:
  Campbell, J.Y. & Vuolteenaho, T. (2004). Bad Beta, Good Beta.
  American Economic Review, 94(5), 1249-1275.
  (Equity risk premium and earnings yields relative to bond yields.)

  Damodaran, A. (2012). Equity Risk Premiums (ERP): Determinants,
  Estimation and Implications. Stern School of Business.
  (Framework for ERP as earnings yield minus risk-free rate.)

Usage:
    # Backtest US stocks (default)
    python3 yield-gap/backtest.py

    # Backtest Indian stocks
    python3 yield-gap/backtest.py --preset india

    # Backtest all exchanges
    python3 yield-gap/backtest.py --global --output results/exchange_comparison.json --verbose

    # Without transaction costs
    python3 yield-gap/backtest.py --no-costs
...
from data_utils import (query_parquet, get_prices, generate_rebalance_dates, filter_returns,
                        entry_buyable, domicile_sql_condition,
...
from cli_utils import (add_common_args, resolve_exchanges, print_header,
                       get_risk_free_rate, get_mktcap_threshold, REGIONAL_RISK_FREE_RATES)

# --- Signal parameters ---
EARNINGS_YIELD_MIN = 0.06    # 6% absolute floor (PE < ~16.7)
SPREAD_ABOVE_RFR = 0.03      # Must offer 3+ percentage points above regional risk-free rate
EARNINGS_YIELD_MAX = 0.50    # Cap at 50%: above this is a data error or distressed company
ROE_MIN = 0.08               # Return on equity > 8% (quality filter)
DE_MAX = 2.0                 # Debt-to-equity < 2.0 (avoid dangerous leverage)
MAX_STOCKS = 30              # Top 30 by earnings yield, equal weight
...
def get_effective_ey_threshold(risk_free_rate):
    """Compute effective earnings yield threshold for a given risk-free rate.

    Threshold = max(EARNINGS_YIELD_MIN, rfr + SPREAD_ABOVE_RFR)
    This ensures:
      - US (rfr=2%): EY > max(6%, 5%) = 6%  (PE < 16.7)
      - Japan (rfr=0.1%): EY > max(6%, 3.1%) = 6%
      - India (rfr=6.5%): EY > max(6%, 9.5%) = 9.5%  (PE < 10.5)
      - Korea (rfr=3%): EY > max(6%, 6%) = 6%
      - Brazil (rfr=10.5%): EY > max(6%, 13.5%) = 13.5% (PE < 7.4)
    """
...
def fetch_data_via_api(client, exchanges, rebalance_dates, verbose=False, domicile=False):
    ...
    Populates tables:
        ...
        metrics_cache(symbol, earningsYield, returnOnEquity, marketCap, filing_epoch, period)
    ...
    # 2. Key metrics: earningsYield, returnOnEquity, marketCap (FY)
    # earningsYield is pre-computed by FMP (= 1/PE, annualized)
    queries = [
        ("metrics_cache", f"""
            SELECT symbol, earningsYield, returnOnEquity, marketCap,
                   dateEpoch as filing_epoch, period
            FROM key_metrics
            WHERE period = 'FY'
              AND earningsYield IS NOT NULL
              AND earningsYield > 0
              AND returnOnEquity IS NOT NULL
              AND {sym_filter_sql}
        """, "key metrics (earnings yield, ROE, market cap)"),
        ("ratios_cache", f"""
...
def screen_stocks(con, target_date, mktcap_min, ey_threshold):
    """Screen for high earnings yield stocks with quality filters.

    Returns list of (symbol, market_cap) tuples sorted by earnings yield DESC.

    Logic:
    - Most recent FY filing <= cutoff date (45-day lag for point-in-time)
    - Earnings yield > ey_threshold (exchange-specific, based on rfr + spread)
    - Earnings yield < 50% (data quality cap)
    - ROE > 8% (profitability quality)
    - D/E < 2.0 (leverage filter)
    - Market cap > exchange-specific threshold
    - Rank: highest earnings yield first (widest gap)
    """
    ...
    rows = con.execute("""
        WITH km AS (
            SELECT symbol, earningsYield, returnOnEquity, marketCap, filing_epoch,
        ...
        SELECT km.symbol, km.marketCap, km.earningsYield
        FROM km
        LEFT JOIN fr ON km.symbol = fr.symbol AND fr.rn = 1
        WHERE km.rn = 1
          AND km.earningsYield > ?
          AND km.earningsYield < ?
          AND km.returnOnEquity > ?
          AND (fr.debtToEquityRatio IS NULL
               OR (fr.debtToEquityRatio >= 0 AND fr.debtToEquityRatio < ?))
          AND km.marketCap > ?
        ORDER BY km.earningsYield DESC
        LIMIT ?
    """, [cutoff_epoch, cutoff_epoch,
          ey_threshold, EARNINGS_YIELD_MAX,
          ROE_MIN, DE_MAX,
          mktcap_min,
          MAX_STOCKS]).fetchall()
...
def run_backtest(con, rebalance_dates, mktcap_min, ey_threshold,
                 use_costs=True, verbose=False, offset_days=1, benchmark_symbol="SPY"):
    """Run Yield Gap backtest. Returns list of period result dicts."""
    results = []
    ...
    for i in range(len(rebalance_dates) - 1):
        ...
        portfolio = screen_stocks(con, entry_date, mktcap_min, ey_threshold)
...
def build_output(metrics, annual, valid, results, universe_name, frequency, periods_per_year,
                 cash_periods, avg_stocks, ey_threshold,
    ...
    return {
        ...
        **period_accounting(
        ...
        "frequency": frequency,
        # What the "spy" block below actually holds. Charts and content read
        # this rather than assuming the S&P 500: after the local-benchmark
        # rerun the field carries whichever index the exchange was measured
        # against. Written by the run that produced the series, never
        # backfilled onto an older one.
        "benchmark": benchmark_symbol,
        "benchmark_name": benchmark_name,
        "universe_basis": "domicile" if domicile else "exchange_listing",
        "avg_stocks_when_invested": round(avg_stocks, 1),
        "period_data": results,
        "ey_threshold_pct": round(ey_threshold * 100, 2),
        "portfolio": fmt(p),
        "spy": fmt(b),
        "comparison": {
...
def run_single(cr, exchanges, universe_name, frequency, use_costs,
               ...
               offset_days=1, benchmark_symbol="SPY", benchmark_name="SPY",
               domicile=False):
    ...
    signal_desc = (f"EY > {ey_threshold*100:.1f}% (rfr={risk_free_rate*100:.1f}%+{SPREAD_ABOVE_RFR*100:.0f}%), "
                   f"ROE > {ROE_MIN*100:.0f}%, D/E < {DE_MAX}, "
                   f"MCap > {mktcap_label} local, top {MAX_STOCKS}")
    print_header("YIELD GAP SCREEN BACKTEST", universe_name, exchanges, signal_desc)
    print(f"  Frequency: {frequency}, Costs: {'size-tiered' if use_costs else 'none'}")
    print(f"  Risk-free rate: {risk_free_rate*100:.1f}%  |  EY threshold: {ey_threshold*100:.1f}%")
    ...
    t1 = time.time()
    results = run_backtest(con, rebalance_dates, mktcap_threshold, ey_threshold,
    ...
    print(format_metrics(metrics, "Yield Gap", benchmark_name))
    ...
    if annual:
        print(f"\n  {'Year':<8} {'Yield Gap':>12} {benchmark_name[:10]:>10} {'Excess':>10}")
        print("  " + "-" * 42)
        for ar in annual:
    ...
    output = build_output(metrics, annual, valid, results, universe_name,
                          frequency, periods_per_year, cash_periods, avg_stocks, ey_threshold,
...
def main():
    parser = argparse.ArgumentParser(description="Yield Gap Screen backtest")
    ...
    if args.cloud:
        ...
        result = run_backtest_cloud("yield-gap", args_str=" ".join(cloud_args),
    ...
    if exchanges is None and universe_name in ("Global", "GLOBAL"):
        ...
        print(f"{'=' * 80}")
        print(f"{'Exchange':<20} {'EY%':>6} {'CAGR':>8} {'Excess':>8} {'Sharpe':>8} "
        ...
        for uni, r in sorted(all_results.items(),
            ...
            if "error" in r or not r.get("portfolio"):
                print(f"{uni:<20} {'ERROR / NO DATA':>50}")
                continue
            p = r.get("portfolio", {})
            c = r.get("comparison", {})
            n = r.get("n_periods", 0)
    ...
    run_single(cr, exchanges, universe_name, frequency, use_costs,
               ...
               offset_days=offset_days, benchmark_symbol=benchmark_symbol,
               benchmark_name=benchmark_name, domicile=args.domicile_filter)
