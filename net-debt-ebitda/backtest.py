#!/usr/bin/env python3
"""
Net Debt to EBITDA Screen Backtest
...
def fetch_data_via_api(client, exchanges, rebalance_dates, verbose=False):
    ...
    Populates tables:
        ...
        metrics_cache(symbol, netDebtToEBITDA, returnOnEquity, marketCap, filing_epoch, period)
    ...
    # 2. Key metrics (net debt/EBITDA, ROE, market cap — all pre-computed in key_metrics)
    print("  Fetching key metrics (Net Debt/EBITDA, ROE, market cap)...")
    metrics_sql = f"""
        SELECT symbol, netDebtToEBITDA, returnOnEquity, marketCap,
               dateEpoch as filing_epoch, period
        FROM key_metrics
        WHERE period = 'FY'
          AND netDebtToEBITDA IS NOT NULL
    ...
    # 3. Prices (only at rebalance dates) — include volume + local benchmark symbols
...
def run_single(cr, exchanges, universe_name, frequency, use_costs,
    ...
    t1 = time.time()
    results = run_backtest(con, rebalance_dates, mktcap_min, use_costs=use_costs,
    ...
    print(format_metrics(metrics, "NetDebt/EBITDA", benchmark_name))
    ...
    if annual:
        print(f"\n  {'Year':<8} {'NetDebt/EBITDA':>15} {benchmark_name:>10} {'Excess':>10}")
        print("  " + "-" * 45)
        for ar in annual:
...
def main():
    parser = argparse.ArgumentParser(description="Net Debt to EBITDA backtest")
    ...
    if args.cloud:
        ...
        result = run_backtest_cloud("net-debt-ebitda", args_str=" ".join(cloud_args),
