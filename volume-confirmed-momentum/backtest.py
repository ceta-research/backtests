...
def fetch_data_via_api(client, exchanges, rebalance_dates, verbose=False, benchmark_symbols=None,
    ...
    # exchange's country, not merely listed on it. Off by default. A screen run
    # on exchange listing admits foreign secondary lines, which outside the US
    # can be most of the universe and can invert the published conclusion.
...
def run_single(cr, exchanges, universe_name, frequency, use_costs,
    ...
    con = fetch_data_via_api(cr, exchanges, rebalance_dates, verbose=verbose,
                             benchmark_symbols=bench_symbols, domicile=domicile)
    if con is None:
...
def main():
    ...
    parser.add_argument("--domicile-filter", action="store_true",
                        help="Restrict the universe to companies domiciled in the exchange's "
                             "country, not merely listed on it (opt-in; default is listing)")
