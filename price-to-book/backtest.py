#!/usr/bin/env python3
"""
Price-to-Book Value Backtest

Annual rebalancing, equal weight, top 30 by lowest P/B.
Fetches data via configurable provider, caches in DuckDB, runs locally.

Signal: P/B 0-1.5, ROE > 8%, Market Cap > local currency threshold
Portfolio: Top 30 by lowest P/B, equal weight. Cash if < 10 qualify.
Rebalancing: Annual (January), 2000-2025.

Academic reference: Fama, E. & French, K. (1992). "The Cross-Section of Expected
Stock Returns." Journal of Finance, 47(2), 427-465. The book-to-market (B/M)
factor is one of the three factors in the Fama-French model, documenting the
value premium in cross-sectional stock returns.

Usage:
    # Backtest US stocks (default)
    python3 price-to-book/backtest.py

    # Backtest Indian stocks
    python3 price-to-book/backtest.py --preset india

    # Backtest all exchanges
    python3 price-to-book/backtest.py --global --output results/exchange_comparison.json --verbose

    # Without transaction costs
    python3 price-to-book/backtest.py --no-costs
...
# --- Signal parameters ---
PB_MIN = 0.0               # Must have positive book value
PB_MAX = 1.5               # Value territory (< 1.5x book)
...
DEFAULT_REBALANCE_MONTHS = [1]  # January only
...
def fetch_data_via_api(client, exchanges, rebalance_dates, verbose=False):
    ...
    Note: priceToBookRatio lives in financial_ratios (not key_metrics).
    ROE and marketCap live in key_metrics.
    """
    if exchanges:
    ...
    # 2. P/B from financial_ratios (FY annual data)
    print("  Fetching P/B ratios (financial_ratios)...")
    ratios_sql = f"""
        SELECT symbol, priceToBookRatio, dateEpoch as filing_epoch, period
        FROM financial_ratios
        WHERE period = 'FY'
          AND priceToBookRatio IS NOT NULL
          AND priceToBookRatio > {PB_MIN}
          AND priceToBookRatio < {PB_MAX}
          AND {sym_filter_sql}
    """
    ...
    for d in rebalance_dates:
        end = d + timedelta(days=11)  # +1 extra day for MOC offset
...
def screen_stocks(con, target_date, mktcap_min):
    """Screen for low P/B value stocks.
...
def run_backtest(con, rebalance_dates, mktcap_min, use_costs=True, verbose=False,
    ...
    """Run P/B backtest. Returns list of period result dicts."""
    results = []
...
def run_single(cr, exchanges, universe_name, frequency, use_costs,
    ...
    signal_desc = (f"P/B {PB_MIN}-{PB_MAX}, ROE > {ROE_MIN*100:.0f}%, "
                   f"MCap > {mktcap_threshold/1e9:.0f}B local, top {MAX_STOCKS}")
    print_header("PRICE-TO-BOOK VALUE BACKTEST", universe_name, exchanges, signal_desc)
    ...
    print(format_metrics(metrics, "Price-to-Book", benchmark_name))
    ...
    if annual:
        ...
        print(f"\n  {'Year':<8} {'P/B':>12} {bench_label:>10} {'Excess':>10}")
        print("  " + "-" * 42)
        for ar in annual:
...
def main():
    parser = argparse.ArgumentParser(description="Price-to-Book value backtest")
    ...
    if args.cloud:
        ...
        result = run_backtest_cloud("price-to-book", args_str=" ".join(cloud_args),
