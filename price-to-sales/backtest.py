#!/usr/bin/env python3
"""
Price-to-Sales (P/S) Value Backtest

Quarterly rebalancing, equal weight, top 30 by lowest P/S.
Fetches data via configurable provider, caches in DuckDB, runs locally.

Signal: P/S 0-1.0, Gross Margin > 20%, Operating Margin > 5%,
        ROE > 10%, Market Cap > local currency threshold
Portfolio: Top 30 by lowest P/S, equal weight. Cash if < 10 qualify.
Rebalancing: Quarterly (Jan/Apr/Jul/Oct), 2000-2025.

Academic reference:
  Barbee, W., Mukherji, S. & Raines, G. (1996). "Do Sales-Price and Debt-Equity
  Explain Stock Returns Better than Book-Market and Firm Size?" Financial Analysts
  Journal, 52(2), 56-60.
  Fisher, K. (1984). "Super Stocks." Dow Jones-Irwin.
  Gray, W. & Vogel, J. (2012). "Analyzing Valuation Measures: A Performance
...
Usage:
    ...
    python3 price-to-sales/backtest.py --global --output results/exchange_comparison.json --verbose

    # Without transaction costs
    python3 price-to-sales/backtest.py --no-costs
...
# --- Signal parameters ---
PS_MAX = 1.0               # P/S < 1.0 (revenue is cheap relative to price)
PS_MIN = 0.0               # Must be positive
GROSS_MARGIN_MIN = 0.20    # Gross margin > 20% (not a margin-destroying business)
OP_MARGIN_MIN = 0.05       # Operating margin > 5% (operationally profitable)
ROE_MIN = 0.10             # Return on equity > 10% (quality filter)
MAX_STOCKS = 30            # Top 30 by P/S, equal weight
...
def fetch_data_via_api(client, exchanges, rebalance_dates, verbose=False):
    ...
    Populates tables:
        ...
        ratios_cache(symbol, priceToSalesRatio, grossProfitMargin,
                     operatingProfitMargin, filing_epoch, period)
    ...
    # 2. Key metrics (ROE, market cap) — from key_metrics table
    # Filter for positive ROE upfront to reduce data volume
    queries = [
        ("metrics_cache", f"""
        ...
        # 3. Financial ratios (P/S, gross margin, op margin) — from financial_ratios table
        ("ratios_cache", f"""
            SELECT symbol, priceToSalesRatio, grossProfitMargin, operatingProfitMargin,
                   dateEpoch as filing_epoch, period
            FROM financial_ratios
            WHERE period = 'FY'
              AND priceToSalesRatio IS NOT NULL
              AND priceToSalesRatio > {PS_MIN}
              AND priceToSalesRatio < {PS_MAX}
              AND {sym_filter_sql}
        """, "financial ratios (P/S, gross margin, op margin)"),
    ]
...
def screen_stocks(con, target_date, mktcap_min):
    """Screen for low P/S stocks with quality filters.

    Returns list of (symbol, market_cap) tuples sorted by P/S ascending.
    ...
    rows = con.execute("""
        ...
        WHERE m.rn = 1
          AND r.priceToSalesRatio > ?
          AND r.priceToSalesRatio < ?
          AND r.grossProfitMargin > ?
          AND r.operatingProfitMargin > ?
          AND m.returnOnEquity > ?
          AND m.marketCap > ?
        ORDER BY r.priceToSalesRatio ASC
        LIMIT ?
    """, [cutoff_epoch, cutoff_epoch,
          PS_MIN, PS_MAX, GROSS_MARGIN_MIN, OP_MARGIN_MIN, ROE_MIN, mktcap_min,
          MAX_STOCKS]).fetchall()
...
def run_backtest(con, rebalance_dates, mktcap_min, use_costs=True, verbose=False,
    ...
    """Run P/S backtest. Returns list of period result dicts."""
    results = []
...
def run_single(cr, exchanges, universe_name, frequency, use_costs,
    ...
    signal_desc = (f"P/S {PS_MIN}-{PS_MAX}, GrossMargin > {GROSS_MARGIN_MIN*100:.0f}%, "
                   f"OpMargin > {OP_MARGIN_MIN*100:.0f}%, ROE > {ROE_MIN*100:.0f}%, "
                   f"MCap > {mktcap_threshold/1e9:.0f}B local, top {MAX_STOCKS}")
    print_header("PRICE-TO-SALES BACKTEST", universe_name, exchanges, signal_desc)
    exec_model = "same-bar (close)" if offset_days == 0 else "next-bar (MOC)"
    ...
    print(format_metrics(metrics, "Price-to-Sales", benchmark_name))
    ...
    if annual:
        bench_label = benchmark_name[:8] if len(benchmark_name) > 8 else benchmark_name
        print(f"\n  {'Year':<8} {'P/S':>12} {bench_label:>10} {'Excess':>10}")
        print("  " + "-" * 42)
        for ar in annual:
...
def main():
    parser = argparse.ArgumentParser(description="Price-to-Sales value backtest")
    ...
    if args.cloud:
        ...
        result = run_backtest_cloud("price-to-sales", args_str=" ".join(cloud_args),
