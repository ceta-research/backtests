#!/usr/bin/env python3
"""
High Dividend Yield Quality Backtest

Annual rebalancing (July), equal weight, top 30 by highest dividend yield with quality filters.
Fetches data via API, caches in DuckDB, runs locally.

Signal: DivYield 4-15%, Payout < 80%, FCF > 0, ROE > 8%, D/E < 2.0, MCap > threshold
Portfolio: Top 30 by highest dividend yield, equal weight. Cash if < 10 qualify.
Rebalancing: Annual (July), 2000-2025.

Academic reference: Fama, E. & French, K. (1998) "Value versus Growth: The International
...
Usage:
    ...
    python3 high-yield/backtest.py --no-costs
"""
...
# --- Signal parameters ---
YIELD_MIN = 0.04           # Minimum dividend yield 4%
YIELD_MAX = 0.15           # Maximum dividend yield 15% (above = artifact or distress)
PAYOUT_MAX = 0.80          # Payout ratio cap (80% of earnings)
...
def screen_stocks(con, target_date, mktcap_min):
    ...
    Returns list of (symbol, market_cap) tuples sorted by dividend yield DESC.
    """
    ...
    rows = con.execute("""
        ...
        WHERE fr.rn = 1
          ...
          AND fr.dividendYield <= ?
          AND fr.dividendPayoutRatio >= ?
          AND fr.dividendPayoutRatio <= ?
          AND cf.freeCashFlow > ?
          AND km.returnOnEquity > ?
          AND (fr.debtToEquityRatio < ? OR fr.debtToEquityRatio IS NULL)
          AND km.marketCap > ?
        ORDER BY fr.dividendYield DESC
        LIMIT ?
    """, [cutoff_epoch, cutoff_epoch, cutoff_epoch,
          YIELD_MIN, YIELD_MAX, PAYOUT_MIN, PAYOUT_MAX,
          FCF_MIN, ROE_MIN, DE_MAX, mktcap_min, MAX_STOCKS]).fetchall()
...
def run_backtest(con, rebalance_dates, mktcap_min, use_costs=True, verbose=False,
    ...
    """Run High Yield backtest. Returns list of period result dicts."""
    results = []
...
def run_single(cr, exchanges, universe_name, frequency, use_costs,
    ...
    signal_desc = (f"DivYield {YIELD_MIN*100:.0f}-{YIELD_MAX*100:.0f}%, "
                   f"Payout < {PAYOUT_MAX*100:.0f}%, FCF > 0, "
                   f"ROE > {ROE_MIN*100:.0f}%, D/E < {DE_MAX:.1f}, "
                   f"MCap > {mktcap_label} local, top {MAX_STOCKS}")
    print_header("HIGH DIVIDEND YIELD QUALITY BACKTEST", universe_name, exchanges, signal_desc)
    ...
    print(format_metrics(metrics, "High Yield", benchmark_name))
    ...
    if annual:
        ...
        print(f"\n  {'Year':<8} {'HighYield':>12} {bench_label:>10} {'Excess':>10}")
        print("  " + "-" * 42)
        for ar in annual:
...
def main():
    parser = argparse.ArgumentParser(description="High Dividend Yield Quality backtest")
    ...
    if args.cloud:
        ...
        result = run_backtest_cloud("high-yield", args_str=" ".join(cloud_args),
