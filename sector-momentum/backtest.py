...
Quarterly rebalancing, equal weight. Buys stocks in the best-performing sectors
...
Signal: Rank sectors by equal-weighted 12-month trailing return.
        Buy all qualifying stocks in the top N_BEST_SECTORS sectors.
...
demonstrate that industry-level momentum explains a substantial portion of individual
stock momentum. Buying top-performing industries captures the persistence of sector
trends driven by earnings momentum, capital flows, and analyst attention.

Usage:
    python3 sector-momentum/backtest.py                                   # US default
    python3 sector-momentum/backtest.py --preset india                    # India
    python3 sector-momentum/backtest.py --preset us --n-best 3            # Top 3 sectors
    python3 sector-momentum/backtest.py --global --output results/exchange_comparison.json
...
# --- Signal parameters ---
N_BEST_SECTORS = 2            # Top N sectors by trailing 12-month return
...
def fetch_data(client, exchanges, mktcap_min, benchmark_symbol="SPY", verbose=False,
    ...
    """Fetch sector mappings and prices for sector momentum backtest.
    ...
    """
    # Always fetch SPY + local benchmark for comparison
    bench_set = {"'SPY'"}
    if benchmark_symbol and benchmark_symbol != "SPY":
        bench_set.add(f"'{benchmark_symbol}'")
    ...
    # (79.5%) are funds. When Financial Services lands in the top 2 the
    ...
    if exchanges:
        ...
        price_sym_filter = (
            ...
            f"OR symbol IN ({bench_list}))"
        )
    else:
        mktcap_filter = f"marketCap > {mktcap_min}{extra}"
        price_sym_filter = (
            f"(symbol IN (SELECT DISTINCT symbol FROM profile "
            f"WHERE marketCap > {mktcap_min}{extra}) OR symbol IN ({bench_list}))"
        )
...
def screen_sectors(con, target_date, n_best=N_BEST_SECTORS):
    """Identify best N sectors and their stocks at target_date.
    ...
    Returns:
        list of (symbol, sector, recent_price, market_cap) for stocks in best N sectors
    ...
    rows = con.execute(f"""
        ...
        WHERE rs.rank_best <= {n_best}
          AND sr.return_12m BETWEEN -0.99 AND {SECTOR_RETURN_MAX}
        ORDER BY rs.rank_best ASC, sr.symbol ASC
    """).fetchall()
...
def get_prices_at(con, symbols, year, month, offset_days=0):
    """Get prices in the given month/year for a list of symbols.
    offset_days=0: first available price (used for signal computation and exit).
    offset_days=1: second available price (MOC execution — enter next trading day)."""
    if not symbols:
        return {}
    sym_list = ", ".join(f"'{s}'" for s in symbols)
    row_num = offset_days + 1  # row 1 for same-day, row 2 for next-day
    ...
    if offset_days > 0:
        ...
        found = {r[0]: r[1] for r in result}
        missing = [s for s in symbols if s not in found]
        if missing:
            miss_list = ", ".join(f"'{s}'" for s in missing)
            fallback = con.execute(f"""
                SELECT symbol, adjClose
                FROM prices_cache
                WHERE symbol IN ({miss_list})
                  AND EXTRACT(YEAR FROM trade_date) = {year}
                  AND EXTRACT(MONTH FROM trade_date) = {month}
                QUALIFY ROW_NUMBER() OVER (PARTITION BY symbol ORDER BY trade_date ASC) = 1
            """).fetchall()
...
def run_backtest(con, rebalance_dates, use_costs=True, verbose=False, n_best=N_BEST_SECTORS,
    ...
    FMP reports marketCap in, so a non-US name is charged its true size tier."""
    results = []
    ...
    for i in range(len(rebalance_dates) - 1):
        ...
        # Screen: identify best sectors and their stocks at entry date (signal uses first price)
        portfolio_rows, n_qualifying = screen_sectors(con, entry_date, n_best=n_best)

        # Local benchmark return for this period
        ...
        if n_qualifying < MIN_QUALIFYING_SECTORS or len(portfolio_rows) < MIN_PORTFOLIO_STOCKS:
            ...
            results.append({
                ...
                "stocks_held": 0,
                "screened": len(portfolio_rows),
                "entry_buyable": None,
                "min_stocks": MIN_PORTFOLIO_STOCKS,
                "sectors_selected": "",
        ...
        # Get entry and exit prices for portfolio
        symbols = [r[0] for r in portfolio_rows]
        mcaps = {r[0]: r[3] for r in portfolio_rows}  # current profile market cap
        # Entry prices with offset for MOC execution (offset_days=1 = next-day close)
        ...
        if verbose:
            ...
            if bench_return is not None:
                excess_str = f"  ex={((port_return - bench_return) * 100):+.1f}%"
            print(f"    {entry_date}: {len(clean)} stocks [{sectors_selected}] "
                  f"port={port_return * 100:.1f}%, bench={bench_str}{excess_str}")

    return results


def build_output(metrics, annual, valid, results, universe_name, frequency,
                 periods_per_year, cash_periods, avg_stocks, n_best=N_BEST_SECTORS,
...
def run_single(cr, exchanges, universe_name, frequency, use_costs,
    ...
    exec_model = "next-day close (MOC)" if offset_days == 1 else "same-day close (biased)"
    fx_per_usd = get_fx_per_usd(exchanges)

    signal_desc = (f"Top {n_best} sectors by 12M trailing return, "
                   f"MCap > {mktcap_label} local, equal weight")
    print_header("SECTOR MOMENTUM ROTATION BACKTEST", universe_name, exchanges, signal_desc)
    print(f"  Frequency: {frequency}, Costs: {'size-tiered' if use_costs else 'none'}"
          f"{f' (tiers x{fx_per_usd:g} local/USD)' if use_costs else ''}")
    ...
    con = fetch_data(cr, exchanges, mktcap_min, benchmark_symbol=benchmark_symbol,
                     verbose=verbose, domicile=domicile, exclude_funds=exclude_funds)
    if con is None:
    ...
    if sector_freq:
        print(f"\n  Sector frequency (quarters in top {n_best}):")
    ...
    if annual:
        ...
        print(f"\n  {'Year':<8} {'SectorMom':>12} {bench_col:>10} {'Excess':>10}")
        print("  " + "-" * 44)
        for ar in annual:
    ...
    output = build_output(metrics, annual, valid, results, universe_name,
                          ...
                          n_best=n_best, benchmark_symbol=benchmark_symbol,
...
def main():
    ...
    parser.add_argument("--exclude-funds", action="store_true",
                        help="Exclude closed-end funds and ETFs (isFund/isEtf). "
                             "Diagnostic, default OFF: published numbers use the "
                             "full listed universe.")
    parser.add_argument("--domicile-filter", action="store_true",
                        help="Restrict the universe to companies headquartered in "
                             "the exchange's home country. Default OFF.")
    args = parser.parse_args()

    if args.cloud:
        from cloud_runner import run_backtest_cloud
        cloud_args = [a for a in sys.argv[1:] if a != "--cloud"]
        result = run_backtest_cloud("sector-momentum", args_str=" ".join(cloud_args),
