#!/usr/bin/env python3
"""
Price-to-Tangible-Book Backtest

Annual rebalancing (July), equal weight, top 30 by lowest P/TBV.
Fetches data via configurable provider, caches in DuckDB, runs locally.

Signal: Low P/TBV ratio (positive tangible book only), quality filters:
        ROE > 8%, ROA > 3%, OPM > 10%, MCap > exchange-specific threshold.

Tangible equity = totalStockholdersEquity - goodwill - intangibleAssets
P/TBV = marketCap / tangible_equity (both in local currency)

Academic reference: Fama & French (1992) "The Cross-Section of Expected
...
ROA_MIN = 0.03             # Return on assets > 3%
...
def fetch_data_via_api(client, exchanges, rebalance_dates, verbose=False):
    ...
    Populates tables:
        ...
        balance_cache(symbol, totalStockholdersEquity, goodwill, intangibleAssets, filing_epoch)
        metrics_cache(symbol, returnOnEquity, returnOnAssets, marketCap, filing_epoch)
        ratios_cache(symbol, operatingProfitMargin, filing_epoch)
    ...
    # 2. Balance sheet (FY): equity, goodwill, intangibles for P/TBV computation
    queries = [
        ("balance_cache", f"""
            SELECT symbol,
            ...
            WHERE period = 'FY'
              ...
              AND {sym_filter_sql}
        """, "balance sheet (equity, goodwill, intangibles)"),
        ("metrics_cache", f"""
...
def screen_stocks(con, target_date, mktcap_min):
    """Screen for low P/TBV quality stocks.

    Computes P/TBV = marketCap / tangible_equity.
    Returns list of (symbol, market_cap) tuples sorted by P/TBV ASC.
    """
    ...
    rows = con.execute("""
        WITH bs AS (
            SELECT symbol,
                   ...
                   goodwill,
                   intangibleAssets,
                   (totalStockholdersEquity - goodwill - intangibleAssets) AS tangible_equity,
                   filing_epoch,
        ...
        SELECT bs.symbol,
               km.marketCap,
               km.marketCap / NULLIF(bs.tangible_equity, 0) AS p_tbv
        FROM bs
...
def run_backtest(con, rebalance_dates, mktcap_min, use_costs=True, verbose=False,
    ...
    """Run P/TBV backtest. Returns list of period result dicts."""
    results = []
...
def run_single(cr, exchanges, universe_name, frequency, use_costs,
    ...
    signal_desc = (f"P/TBV ASC, ROE > {ROE_MIN*100:.0f}%, ROA > {ROA_MIN*100:.0f}%, "
                   f"OPM > {OPM_MIN*100:.0f}%, MCap > {mktcap_label} local, top {MAX_STOCKS}")
    print_header("PRICE-TO-TANGIBLE-BOOK BACKTEST", universe_name, exchanges, signal_desc)
    ...
    print(format_metrics(metrics, "P/TBV", benchmark_name))
    ...
    if annual:
        ...
        print(f"\n  {'Year':<8} {'P/TBV':>12} {bench_col:>10} {'Excess':>10}")
        print("  " + "-" * 42)
        for ar in annual:
...
def main():
    parser = argparse.ArgumentParser(description="Price-to-Tangible-Book backtest")
    ...
    if args.cloud:
        ...
        result = run_backtest_cloud("tangible-book", args_str=" ".join(cloud_args),
