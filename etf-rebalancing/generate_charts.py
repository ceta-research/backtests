#!/usr/bin/env python3
"""
Generate charts for ETF Rebalancing Drag backtest results.
...
Usage:
    python3 etf-rebalancing/generate_charts.py
"""
...
STRATEGY_NAME = "Low Rebalancing Exposure"
STRATEGY_COLOR = "#7B1FA2"  # Purple (distinct from anti-crowding blue)
SPY_COLOR = "#FF9800"
...
def comparison_cagr_chart(all_results, output_path):
    ...
    for uni, r in all_results.items():
        ...
        avg_ratio = r.get("avg_ownership_ratio", 0)
        if cagr is not None:
            data.append((uni, cagr, excess or 0, avg_ratio))
    ...
    for i, (name, cagr, excess, ratio) in enumerate(data):
