#!/usr/bin/env python3
"""
DCF Threshold Value Strategy Backtest

Gordon Growth Model DCF screen with quality filters to avoid value traps.

Signal:
    FCF / MarketCap >= 8.78%  (= 20% discount to DCF intrinsic value)
    + Operating cash flow > 0
    + ROE > 8%
    + D/E < 1.5 (or null — not penalized)

DCF math:
    DCF_value  = FCF * (1+g)/(r-g) = FCF * 13.67
    discount%  = 1 - marketCap / (FCF * 13.67)
    Threshold: FCF/marketCap >= (1 + 0.20) / 13.67 = 8.78%

Parameters:
    g = 2.5%, r = 10%, multiple = 13.67, threshold = 20%
    Portfolio: Top 30 by FCF yield, equal weight
    Rebalancing: Annual (January)
    Filing lag: 45 days. Staleness: 18 months.

Distinct from dcf-discount:
    dcf-discount: No quality filters, top 50, April rebalance.
    dcf-threshold: ROE + D/E + OCF filters prevent value traps, top 30, January.

Usage:
    python3 dcf-threshold/backtest.py                    # US default
    python3 dcf-threshold/backtest.py --preset india     # India
    python3 dcf-threshold/backtest.py --preset germany   # Germany XETRA
    python3 dcf-threshold/backtest.py --global --output results/exchange_comparison.json
    python3 dcf-threshold/backtest.py --no-costs         # Without transaction costs
    python3 dcf-threshold/backtest.py --cloud            # Run on Ceta Research cloud
...
from data_utils import query_parquet, get_prices, generate_rebalance_dates, filter_returns, entry_buyable, remove_price_oscillations
...
DISCOUNT_THRESHOLD = 0.20   # 20% margin of safety
FCF_YIELD_MIN = (1 + DISCOUNT_THRESHOLD) / DCF_MULTIPLE  # ~8.78%

# Quality filters
ROE_MIN = 0.08   # Return on equity > 8%
DE_MAX = 1.5     # Debt-to-equity < 1.5 (NULL = allowed through)
...
def fetch_data_via_api(client, exchanges, rebalance_dates, verbose=False):
    ...
    Populates tables:
        cashflow_cache(symbol, freeCashFlow, operatingCashFlow, filing_epoch, period)
    ...
    # 2. Cash flow data (FCF + OCF in one fetch)
    queries = [
        ("cashflow_cache", f"""
...
def screen_stocks(con, target_date, mktcap_min):
    """Screen for quality-filtered, deep DCF-discount stocks.

    Signal:
      - FCF/MarketCap >= 8.78% (= 20% DCF discount via Gordon Growth)
      - OperatingCashFlow > 0 (genuine cash generation)
      - ROE > 8% (profitable)
      - D/E < 1.5 (not over-leveraged; NULL is allowed)
    ...
    rows = con.execute("""
        ...
        SELECT cf.symbol, km.marketCap,
            cf.freeCashFlow / km.marketCap AS fcf_yield
        FROM cf
        JOIN km ON cf.symbol = km.symbol AND km.rn = 1
        LEFT JOIN ra ON cf.symbol = ra.symbol AND ra.rn = 1
        WHERE cf.rn = 1
          AND km.marketCap >= ?
          AND cf.freeCashFlow / km.marketCap >= ?
...
def run_backtest(con, rebalance_dates, mktcap_min, use_costs=True, verbose=False):
    """Run DCF Threshold backtest. Returns list of period result dicts."""
    results = []
...
def run_single(cr, exchanges, universe_name, frequency, use_costs,
    ...
    signal_desc = (
        f"FCF yield >= {FCF_YIELD_MIN*100:.2f}% (= {DISCOUNT_THRESHOLD*100:.0f}% DCF discount), "
        f"ROE > {ROE_MIN*100:.0f}%, D/E < {DE_MAX}, OCF > 0, "
        f"MCap >= {mktcap_threshold/1e9:.0f}B local, top {MAX_STOCKS}"
    )
    print_header("DCF THRESHOLD BACKTEST", universe_name, exchanges, signal_desc)
    ...
    print(format_metrics(metrics, "DCF Threshold", "S&P 500"))
    ...
    if annual:
        print(f"\n  {'Year':<8} {'DCF Threshold':>15} {'SPY':>10} {'Excess':>10}")
        print("  " + "-" * 46)
        for ar in annual:
            print(f"  {ar['year']:<8} {ar['portfolio']*100:>14.1f}% "
...
def main():
    parser = argparse.ArgumentParser(description="DCF Threshold Value Strategy backtest")
    ...
    if args.cloud:
        ...
        result = run_backtest_cloud("dcf-threshold", args_str=" ".join(cloud_args),
    ...
    if exchanges is None and universe_name in ("Global", "GLOBAL"):
        ...
        presets_to_run = [
            ...
            ("singapore",    ["SES"]),
            ("southafrica",  ["JNB"]),
            # ASX / SAO excluded — adjClose split artifacts (DATA_QUALITY_ISSUES.md)
        ]
