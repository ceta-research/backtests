...
Annual rebalancing (July), equal weight, top 30 by FCF coverage ratio.
Fetches data via API, caches in DuckDB, runs locally.

Signal: freeCashFlow / ABS(commonDividendsPaid) between 1.5x and 20x,
        dividendYield > 2%, marketCap > exchange-specific threshold.
Portfolio: Top 30 by coverage DESC, equal weight. Cash if < 10 qualify.
Rebalancing: Annual (July), 2000-2025.

Academic reference: Benartzi, Michaely & Thaler (1997) "Do Changes in Dividends
Signal the Future or the Past?", Journal of Finance 52(3).

Usage:
    python3 dividend-coverage/backtest.py
    python3 dividend-coverage/backtest.py --preset india
    python3 dividend-coverage/backtest.py --global --output results/exchange_comparison.json --verbose
    python3 dividend-coverage/backtest.py --no-costs
"""
...
# --- Signal parameters ---
COVERAGE_MIN = 1.5         # Minimum FCF / ABS(dividends paid) ratio
COVERAGE_MAX = 20.0        # Maximum (>20x usually means token dividend)
YIELD_MIN = 0.02           # Minimum dividend yield (2%)
MAX_STOCKS = 30
MIN_STOCKS = 10
DEFAULT_FREQUENCY = "annual"
DEFAULT_REBALANCE_MONTHS = [7]  # July (annual filings available)
...
def fetch_data_via_api(client, exchanges, rebalance_dates, verbose=False):
    """Fetch cash flow, financial ratios, and price data into DuckDB."""
    if exchanges:
    ...
    # 2. Cash flow statement (FCF + dividends paid)
    queries = [
        ("cashflow_cache", f"""
            SELECT symbol, freeCashFlow, commonDividendsPaid,
                   dateEpoch as filing_epoch, period
            FROM cash_flow_statement
            WHERE period = 'FY'
              AND freeCashFlow IS NOT NULL
              AND commonDividendsPaid IS NOT NULL
              AND commonDividendsPaid < 0
              AND {sym_filter_sql}
    ...
    for d in rebalance_dates:
        end = d + timedelta(days=11)  # +1 extra for offset_days MOC execution
        date_conditions.append(f"(date >= '{d.isoformat()}' AND date <= '{end.isoformat()}')")
    date_filter = " OR ".join(date_conditions)

    # Include local benchmark symbol(s) alongside SPY
    bench_symbols = {"'SPY'"}
    if exchanges:
        for ex in exchanges:
...
def screen_stocks(con, target_date, mktcap_min):
    """Screen for dividend coverage stocks.

    Signal: FCF / ABS(commonDividendsPaid) between COVERAGE_MIN and COVERAGE_MAX
    Filters: dividendYield > YIELD_MIN, marketCap > mktcap_min

    Returns list of (symbol, market_cap) sorted by coverage DESC.
    """
    ...
    rows = con.execute("""
        ...
        SELECT cf.symbol, km.marketCap,
               cf.freeCashFlow / NULLIF(ABS(cf.commonDividendsPaid), 0) AS coverage
        FROM cf
        JOIN fr ON cf.symbol = fr.symbol AND fr.rn = 1
        JOIN km ON cf.symbol = km.symbol AND km.rn = 1
        WHERE cf.rn = 1
          AND cf.freeCashFlow > 0
          AND cf.commonDividendsPaid < 0
          AND cf.freeCashFlow / NULLIF(ABS(cf.commonDividendsPaid), 0) >= ?
          AND cf.freeCashFlow / NULLIF(ABS(cf.commonDividendsPaid), 0) <= ?
    ...
    """, [cutoff_epoch, cutoff_epoch, cutoff_epoch,
          COVERAGE_MIN, COVERAGE_MAX, YIELD_MIN, mktcap_min, MAX_STOCKS]).fetchall()
...
def run_backtest(con, rebalance_dates, mktcap_min, use_costs=True, verbose=False,
    ...
    """Run Dividend Coverage backtest. Returns list of period result dicts."""
    results = []
...
def run_single(cr, exchanges, universe_name, frequency, use_costs,
    ...
    signal_desc = (f"Coverage {COVERAGE_MIN}-{COVERAGE_MAX}x, "
                   f"Yield > {YIELD_MIN*100:.0f}%, "
                   f"MCap > {mktcap_label} local, top {MAX_STOCKS}")
    benchmark_symbol, benchmark_name = get_local_benchmark(exchanges)
    exec_model = "same-day" if offset_days == 0 else "next-day MOC"
    print_header("DIVIDEND COVERAGE BACKTEST", universe_name, exchanges, signal_desc)
    ...
    print(format_metrics(metrics, "DivCoverage", benchmark_name))
    ...
    if annual:
        bench_label = benchmark_name[:8]
        print(f"\n  {'Year':<8} {'DivCov':>12} {bench_label:>10} {'Excess':>10}")
        print("  " + "-" * 42)
        for ar in annual:
...
def main():
    parser = argparse.ArgumentParser(description="Dividend Coverage backtest")
    ...
    if args.cloud:
        ...
        result = run_backtest_cloud("dividend-coverage", args_str=" ".join(cloud_args),
