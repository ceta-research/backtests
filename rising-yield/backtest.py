#!/usr/bin/env python3
"""
Rising Dividend Yield Backtest

Annual rebalancing (July), equal weight, stocks with 3 consecutive years of
rising dividend yield driven by dividend growth (not price decline).

Signal:
  - 3 consecutive FY periods where dividendYield increased year-over-year
  - Driver = "Dividend Growth": DPS also increased (bullish signal)
  - Quality filters: ROE > 10%, Payout < 75%, Market Cap > threshold
  - Portfolio: Top 30 by 2-year yield expansion, equal weight
  - Cash if < 10 qualify

Academic reference:
  Campbell, J.Y. & Shiller, R.J. (1988) "The Dividend-Price Ratio and
  Expectations of Future Dividends and Discount Factors", RFS 1(3): 195-228.
  Cochrane, J.H. (2008) "The Dog That Did Not Bark: A Defense of Return
  Predictability", RFS 21(4): 1533-1575.

Usage:
    python3 rising-yield/backtest.py
    python3 rising-yield/backtest.py --preset india
    python3 rising-yield/backtest.py --global --output results/exchange_comparison.json --verbose
    python3 rising-yield/backtest.py --no-costs
"""
...
# --- Signal parameters ---
CONSECUTIVE_YEARS = 3      # Minimum consecutive FY periods of rising yield
ROE_MIN = 0.10             # Return on equity minimum
PAYOUT_MAX = 0.75          # Payout ratio cap
PAYOUT_MIN = 0.0           # Exclude negative payout
MAX_STOCKS = 30
MIN_STOCKS = 10
DEFAULT_FREQUENCY = "annual"
DEFAULT_REBALANCE_MONTHS = [7]  # July (FY data available by then)
...
def fetch_data_via_api(client, exchanges, rebalance_dates, verbose=False):
    """Fetch dividend yield, quality metrics, and prices into DuckDB."""
    if exchanges:
    ...
    # 2. Financial ratios - core data for yield trajectory
    print("  Fetching financial ratios (dividend yield, DPS, payout)...")
    ratios_sql = f"""
        SELECT symbol, dividendYield, dividendPerShare, dividendPayoutRatio,
               date, dateEpoch as filing_epoch, period
    ...
    if count == 0:
        print("  No dividend yield data. Skipping.")
        con.close()
        return None

    # Precompute yield trajectory with lags (deduplicated by symbol+fiscalYear)
    con.execute("""
        CREATE TABLE yield_trajectory AS
        WITH deduped AS (
            SELECT symbol, dividendYield, dividendPerShare, dividendPayoutRatio,
                   filing_epoch,
                   EXTRACT(YEAR FROM CAST(date AS DATE))::INTEGER AS fiscal_year,
                   ROW_NUMBER() OVER (
                       PARTITION BY symbol, EXTRACT(YEAR FROM CAST(date AS DATE))::INTEGER
                       ORDER BY filing_epoch DESC
                   ) AS rn
            FROM ratios_cache
        ),
        yearly AS (
            SELECT symbol, fiscal_year, dividendYield, dividendPerShare,
                   dividendPayoutRatio, filing_epoch,
                   LAG(dividendYield, 1) OVER (PARTITION BY symbol ORDER BY fiscal_year) AS yield_1yr,
                   LAG(dividendYield, 2) OVER (PARTITION BY symbol ORDER BY fiscal_year) AS yield_2yr,
                   LAG(dividendPerShare, 1) OVER (PARTITION BY symbol ORDER BY fiscal_year) AS dps_1yr,
                   LAG(dividendPerShare, 2) OVER (PARTITION BY symbol ORDER BY fiscal_year) AS dps_2yr
            FROM deduped
            WHERE rn = 1
        )
        SELECT *,
            CASE
                WHEN dividendPerShare > dps_1yr AND dps_1yr > dps_2yr
                    THEN 'Consecutive DPS Growth'
                WHEN dividendPerShare > dps_1yr
                    THEN 'Partial DPS Growth'
                ELSE 'Price Decline Driver'
            END AS yield_driver,
            (dividendYield - yield_2yr) AS yield_change_2yr
        FROM yearly
        WHERE yield_2yr IS NOT NULL
          AND dividendYield > yield_1yr
          AND yield_1yr > yield_2yr
    """)

    traj_count = con.execute("SELECT COUNT(*) FROM yield_trajectory").fetchone()[0]
    print(f"    -> {traj_count} rising-yield observations (3yr consecutive)")

    # 3. Key metrics (ROE, market cap)
    ...
    # Include SPY + local benchmark symbols
    bench_symbols = {"'SPY'"}
    if exchanges:
        for ex in exchanges:
        ...
        WHERE ({date_filter})
          AND (
            ...
            OR symbol IN (
                ...
                WHERE period = 'FY' AND dividendYield > 0
...
def screen_stocks(con, target_date, mktcap_min):
    """Screen for rising dividend yield stocks with quality + driver filters.

    Signal: 3 consecutive FY years of rising dividendYield, where the driver
    is dividend growth (DPS increased), not price decline.

    Returns list of (symbol, market_cap) sorted by yield_change_2yr DESC.
    """
    max_year = target_date.year - 1  # Last complete FY before rebalance
    cutoff_epoch = int(datetime.combine(
        target_date - timedelta(days=45), datetime.min.time()
    ).timestamp())

    rows = con.execute("""
        WITH latest_rising AS (
            SELECT symbol, fiscal_year, dividendYield, yield_change_2yr,
                   yield_driver, dividendPayoutRatio, filing_epoch,
                   ROW_NUMBER() OVER (
                       PARTITION BY symbol ORDER BY fiscal_year DESC
                   ) AS rn
            FROM yield_trajectory
            WHERE fiscal_year <= ?
              AND filing_epoch <= ?
              AND yield_driver IN ('Consecutive DPS Growth', 'Partial DPS Growth')
        ),
        km AS (
        ...
        SELECT lr.symbol, km.marketCap, lr.yield_change_2yr, lr.yield_driver
        FROM latest_rising lr
        JOIN km ON lr.symbol = km.symbol AND km.rn = 1
        WHERE lr.rn = 1
          AND km.returnOnEquity > ?
          AND km.marketCap > ?
          AND (lr.dividendPayoutRatio >= ? AND lr.dividendPayoutRatio <= ?)
        ORDER BY lr.yield_change_2yr DESC
        LIMIT ?
    """, [max_year, cutoff_epoch, cutoff_epoch,
          ROE_MIN, mktcap_min, PAYOUT_MIN, PAYOUT_MAX, MAX_STOCKS]).fetchall()
...
def run_backtest(con, rebalance_dates, mktcap_min, use_costs=True, verbose=False,
    ...
    """Run Rising Yield backtest. Returns list of period result dicts."""
    results = []
...
def run_single(cr, exchanges, universe_name, frequency, use_costs,
    ...
    signal_desc = (f"3yr rising yield (DPS growth driver), "
                   f"ROE > {ROE_MIN*100:.0f}%, Payout < {PAYOUT_MAX*100:.0f}%, "
                   f"MCap > {mktcap_label} local, top {MAX_STOCKS}")
    print_header("RISING DIVIDEND YIELD BACKTEST", universe_name, exchanges, signal_desc)
    exec_model = "same-bar" if offset_days == 0 else "next-day MOC"
    ...
    print(format_metrics(metrics, "RisingYield", benchmark_name))
    ...
    if annual:
        ...
        print(f"\n  {'Year':<8} {'RisingYld':>12} {bench_col:>10} {'Excess':>10}")
        print("  " + "-" * 42)
        for ar in annual:
...
def main():
    parser = argparse.ArgumentParser(description="Rising Dividend Yield backtest")
    ...
    if args.cloud:
        ...
        result = run_backtest_cloud("rising-yield", args_str=" ".join(cloud_args),
