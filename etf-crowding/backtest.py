#!/usr/bin/env python3
"""
ETF Anti-Crowding Backtest

Annual rebalancing (July), equal weight, top 30 least-crowded quality stocks.
Uses current ETF holdings snapshot as crowding signal + point-in-time financials.

Signal: Least ETF ownership among quality stocks (ROE > 10%, P/E 0-40, MCap > threshold)
Portfolio: Bottom 30 by ETF count (least crowded), equal weight. Cash if < 10 qualify.
Rebalancing: Annual (July), 2005-2025.

Academic basis: Ben-David, Franzoni & Moussawi (2018) showed ETF ownership amplifies
volatility by 16% per SD. Israeli et al. (2017) found reduced price informativeness.
Hypothesis: Less-crowded quality stocks have better risk-adjusted returns because
their prices reflect fundamentals more than ETF flow mechanics.

IMPORTANT CAVEAT: ETF holdings data (etf_holder) is a current snapshot, not historical.
Crowding classifications are applied retrospectively across all backtest periods.
The quality filters (ROE, P/E, market cap) use point-in-time FY data. This means the
crowding signal has look-ahead bias while the quality filters do not. Results should
be interpreted as "how would a portfolio of currently under-owned quality stocks have
...
def fetch_data_via_api(client, exchanges, rebalance_dates, verbose=False):
    """Fetch ETF crowding scores, financial data, and prices into DuckDB.

    Tables created:
        crowding_cache(symbol VARCHAR, etf_count INTEGER)
    ...
    # 1. ETF crowding scores (current snapshot, joined with profile for exchange filter)
    print("  Fetching ETF crowding scores...")
    crowding_sql = f"""
        SELECT
            eh.asset as symbol,
        ...
        {exchange_where.replace('exchange', 'p.exchange') if exchange_where else ''}
        GROUP BY eh.asset
        HAVING COUNT(DISTINCT eh.symbol) >= {MIN_ETF_COUNT}
    """
    crowding_rows = client.query(crowding_sql, verbose=verbose, timeout=300,
                                 memory_mb=4096, threads=2)
    if not crowding_rows:
        print("  No ETF crowding data for these exchanges.")
        return None

    # Load crowding into DuckDB
    vals = ",".join(f"('{r['symbol']}', {r['etf_count']})" for r in crowding_rows)
    con.execute("CREATE TABLE crowding_cache(symbol VARCHAR, etf_count INTEGER)")
    con.execute(f"INSERT INTO crowding_cache VALUES {vals}")
    print(f"    -> {len(crowding_rows)} stocks with ETF data (>= {MIN_ETF_COUNT} ETFs)")
...
def screen_stocks(con, target_date, mktcap_min):
    """Screen for least-crowded quality stocks at a given date.

    Quality filters use point-in-time FY data (45-day lag).
    Crowding uses current snapshot (static across all periods).

    Returns list of (symbol, market_cap, etf_count) sorted by etf_count ASC.
    """
    ...
    rows = con.execute("""
        ...
        SELECT km.symbol, km.marketCap, cc.etf_count
        FROM km
        JOIN fr ON km.symbol = fr.symbol AND fr.rn = 1
        JOIN crowding_cache cc ON km.symbol = cc.symbol
        WHERE km.rn = 1
          AND km.returnOnEquity > ?
          AND fr.priceToEarningsRatio > ?
          AND fr.priceToEarningsRatio < ?
          AND km.marketCap > ?
        ORDER BY cc.etf_count ASC
        LIMIT ?
...
def run_backtest(con, rebalance_dates, mktcap_min, use_costs=True, verbose=False,
    ...
    """Run ETF Anti-Crowding backtest. Returns list of period result dicts."""
    results = []
...
def build_output(metrics, annual, valid, results, universe_name, frequency,
    ...
    return {
        ...
        **period_accounting(
        ...
        "caveat": "ETF crowding signal uses current snapshot data (look-ahead bias). "
...
def run_single(cr, exchanges, universe_name, frequency, use_costs,
    ...
    signal_desc = (f"Least-crowded (>= {MIN_ETF_COUNT} ETFs), "
                   f"ROE > {ROE_MIN*100:.0f}%, P/E {PE_MIN:.0f}-{PE_MAX:.0f}, "
                   f"MCap > {mktcap_label} local, bottom {MAX_STOCKS} by ETF count")
    print_header("ETF ANTI-CROWDING BACKTEST", universe_name, exchanges, signal_desc)
    ...
    print(format_metrics(metrics, "ETF Anti-Crowding", benchmark_name))
    ...
    invested = [r for r in executed if r["stocks_held"] > 0]
    ...
    if annual:
        bench_short = benchmark_name[:10]
        print(f"\n  {'Year':<8} {'AntiCrowd':>12} {bench_short:>10} {'Excess':>10}")
        print("  " + "-" * 42)
        for ar in annual:
...
def main():
    parser = argparse.ArgumentParser(description="ETF Anti-Crowding backtest")
    ...
    if args.cloud:
        ...
        result = run_backtest_cloud("etf-crowding", args_str=" ".join(cloud_args),
