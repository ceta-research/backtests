#!/usr/bin/env python3
"""
Generate charts for ETF Anti-Crowding backtest results.
...
Usage:
    python3 etf-crowding/generate_charts.py
"""
...
STRATEGY_NAME = "ETF Anti-Crowding"
...
def comparison_cagr_chart(all_results, output_path):
    ...
    for uni, r in all_results.items():
        ...
        avg_etf = r.get("avg_etf_count", 0)
        signal = "Strong" if avg_etf < 30 else ("Moderate" if avg_etf < 80 else "Weak")
        if cagr is not None:
            data.append((uni, cagr, excess or 0, signal))

    data.sort(key=lambda x: x[1], reverse=True)
    names = [d[0] for d in data]
    cagrs = [d[1] for d in data]
    colors = [EXCESS_POS_COLOR if d[2] > 0 else EXCESS_NEG_COLOR for d in data]
    signals = [d[3] for d in data]

    fig, ax = plt.subplots(figsize=(12, 8))
    bars = ax.barh(range(len(names)), cagrs, color=colors, alpha=0.85)

    # Add signal strength labels
    for i, (name, cagr, excess, signal) in enumerate(data):
        label = f" {cagr:.1f}% ({signal})"
...
def main():
    ...
    print("Move charts to blog directories:")
    print("  mv charts/1_us_*.png charts/2_us_*.png -> blogs/us/")
    print("  mv charts/1_india_*.png charts/2_india_*.png -> blogs/india/")
    print("  mv charts/1_germany_*.png charts/2_germany_*.png -> blogs/germany/")
    print("  mv charts/1_china_*.png charts/2_china_*.png -> blogs/china/")
    print("  mv charts/1_comparison_*.png charts/2_comparison_*.png -> blogs/comparison/")
