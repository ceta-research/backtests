...
def fetch_data_via_api(client, exchanges, rebalance_dates, benchmark_symbol="SPY", verbose=False):
    ...
    # Build benchmark symbol list (SPY + local index for non-US exchanges)
    bench_symbols = {"'SPY'"}
    if exchanges:
        for ex in exchanges:
    ...
    if benchmark_symbol and f"'{benchmark_symbol}'" not in bench_symbols:
...
def run_single(cr, exchanges, universe_name, frequency, use_costs,
    ...
    con = fetch_data_via_api(cr, exchanges, rebalance_dates,
                             benchmark_symbol=benchmark_symbol, verbose=verbose)
    if con is None:
    ...
    print(format_metrics(metrics, "Equity Growth", benchmark_name))
    ...
    if annual:
        bench_col = f"{benchmark_name[:8]:>9}"
        print(f"\n  {'Year':<8} {'EquityGrowth':>13} {bench_col} {'Excess':>10}")
        print("  " + "-" * 43)
        for ar in annual:
