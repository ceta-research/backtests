#!/usr/bin/env python3
"""
Generate charts for 12-Month Price Momentum strategy.
...
Charts are saved to price-momentum/charts/ and must be moved to:
  ts-content-creator/content/_current/momentum-01-12-month/blogs/{region}/

Usage:
    cd backtests
    python3 price-momentum/generate_charts.py
"""
...
STRATEGY_COLOR = "#7B1FA2"    # Purple (momentum = energy/speed)
...
def plot_comparison_cagr(all_data, output_path):
    ...
    ax.set_title("12-Month Momentum: CAGR by Exchange (2001–2024)",
                 fontsize=14, fontweight="bold", pad=12)
...
def plot_comparison_drawdown(all_data, output_path):
    ...
    ax.set_title("12-Month Momentum: Max Drawdown by Exchange (2001–2024)",
                 fontsize=14, fontweight="bold", pad=12)
...
# Update after backtest results to include only exchanges with clean data
EXCHANGE_LABELS = {
...
def main():
    ...
    print("Generating 12-Month Price Momentum charts...")
    ...
    print("\nNext: Move charts to:")
    print("  ts-content-creator/content/_current/momentum-01-12-month/blogs/{region}/")
