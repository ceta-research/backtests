#!/usr/bin/env python3
"""
12-Month Price Momentum Backtest

Pure price momentum strategy: buys the top 30 stocks by 12-month price return
within each exchange. Uses the "12-1 momentum" signal (skip last month) from
Jegadeesh & Titman (1993) to avoid short-term reversal contamination.

No financial quality filters — this is pure price signal, not quality+momentum.
Universe filter: Market cap > exchange threshold, Price > $1.

Signal: Top 30 by 12M-1M price return (12M lookback, skip last month)
        Universe: MCap > exchange threshold, Price > $1
...
Usage:
    python3 price-momentum/backtest.py                           # US default
...
SKIP_DAYS = 30          # Skip last month (avoid short-term reversal)
...
MIN_MOMENTUM_PRICE = 1.0 # Skip stocks where momentum-start OR momentum-end price < $1
                         # Sub-$1 historical adjClose is almost always a split-adjustment artifact
MAX_MOMENTUM = 5.0       # Cap 12M-1M momentum signal at 500% (data quality guard)
                         # Extreme momentum (e.g. 10,000%+) is always a price data artifact

# Development compute (max resources)
FETCH_MEMORY_MB = 16384
FETCH_THREADS = 6


def fetch_data_via_api(client, exchanges, rebalance_dates, verbose=False):
    """Fetch all data needed for 12-month price momentum backtest.
    ...
    # Only need marketCap — no other financial fields required for pure momentum
    ...
    # 3. Prices — extended window: momentum lookback start (R-410d) through entry end (R+10d)
    # This single contiguous window per rebalance date covers:
    #   - Momentum start price (12M ago): found in [R-395, R-335]
    #   - Momentum end price (1M ago, skip): found in [R-45, R-15]
    #   - Entry price: found in [R, R+10]
    ...
    # Much more efficient than fetching all exchange profile symbols (e.g. NASDAQ has 14k symbols
    # in profile but only 5.5k have key_metrics — reduces price volume by ~60%).
    exchange_fy_filter = f"AND {sym_filter_sql}" if sym_filter_sql != "1=1" else ""

    # Build benchmark symbol list (SPY + MTUM + local index benchmarks)
    bench_symbols = {"'SPY'", "'MTUM'"}
    if exchanges:
        for ex in exchanges:
...
def compute_momentum(con, symbols, target_date):
    """Compute 12-1 month momentum for a set of symbols.

    12-1 momentum = return from 12 months ago to 1 month ago (skip last month).
    Symbols without sufficient price history are excluded.
    ...
    # Momentum start: ~12M ago
    lookback_start = target_date - timedelta(days=MOMENTUM_DAYS)
    # Momentum end: ~1M ago (skip last month to avoid reversal)
    ...
    for sym in symbols:
        ...
        if not (start and end):
            continue
        # Filter sub-$1 prices at momentum start/end (split-adjustment artifacts in FMP adjClose)
        if start < MIN_MOMENTUM_PRICE or end < MIN_MOMENTUM_PRICE:
            continue
        mom = (end - start) / start
        # Cap extreme momentum (e.g. 100,000%+ is always a data artifact, not a real signal)
        if mom > MAX_MOMENTUM:
            continue
        momentum[sym] = mom
...
def screen_stocks(con, target_date, mktcap_min, verbose=False):
    """Screen for 12M momentum stocks at target_date.

    1. Get universe passing market cap filter (point-in-time)
    2. Compute 12-1M momentum for universe
    3. Select top MAX_STOCKS by momentum

    Returns list of (symbol, market_cap, momentum_12m) sorted by momentum desc.
    """
    eligible = get_eligible_symbols(con, target_date, mktcap_min)
    if not eligible:
        return []

    momentum = compute_momentum(con, list(eligible.keys()), target_date)

    candidates = [
        (sym, eligible[sym], mom)
...
def run_backtest(con, rebalance_dates, mktcap_min, use_costs=True, verbose=False,
    ...
    for i in range(len(rebalance_dates) - 1):
        ...
        if verbose:
            ...
            print(f"    {entry_date}: {len(returns)} stocks (avg 12M-1M={avg_mom:.0f}%), "
...
def run_single(cr, exchanges, universe_name, frequency, use_costs,
    ...
    signal_desc = (
        f"Top {MAX_STOCKS} by 12M-1M price return | MCap>{mktcap_threshold/1e9:.0f}B local"
    )
    print_header("12-MONTH PRICE MOMENTUM BACKTEST", universe_name, exchanges, signal_desc)
    ...
    if annual:
        ...
        print(f"\n  {'Year':<8} {'12M Mom':>10} {bench_col:>10} {'Excess':>10}")
        print("  " + "-" * 40)
        for ar in annual:
...
def main():
    ...
    if args.cloud:
        ...
        result = run_backtest_cloud("price-momentum", args_str=" ".join(cloud_args),
    ...
    if exchanges is None and universe_name in ("Global", "GLOBAL"):
        ...
        # SAO excluded (adjClose artifacts in backtests; fine for event studies).
