#!/usr/bin/env python3
"""
Generate charts for Dividend Growth (Aristocrats) backtest results.
...
Usage:
    cd backtests
    python3 dividend-growth/generate_charts.py
"""
...
def load_results():
    ...
    if not os.path.exists(path):
        ...
        print("Run: python3 dividend-growth/backtest.py --global --output results/exchange_comparison.json")
        sys.exit(1)
    with open(path) as f:
        return json.load(f)
...
def plot_cagr_comparison(all_results):
    ...
    sorted_data = sorted(zip(exchanges, cagrs, colors), key=lambda x: x[1])
    exchanges, cagrs, colors = zip(*sorted_data)
    ...
    ax.set_xlabel("CAGR (%)")
    ax.set_title(f"{STRATEGY_NAME}: CAGR by Exchange", fontsize=14)
...
def main():
    print(f"Generating {STRATEGY_NAME} charts...")
