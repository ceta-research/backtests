...
Annual rebalancing (July), equal weight, top 30 by longest consecutive dividend
increase streak with quality filters.
Fetches data via API, caches in DuckDB, runs locally.

Signal: 5+ consecutive years of annual dividend increases, Payout < 80%, FCF > 0,
        MCap > threshold
Portfolio: Top 30 by streak length DESC, equal weight. Cash if < 10 qualify.
Rebalancing: Annual (July), 2000-2025.

Academic reference: Lintner, J. (1956) "Distribution of Incomes of Corporations
Among Dividends, Retained Earnings, and Taxes", American Economic Review 46(2).
...
# --- Signal parameters ---
MIN_STREAK = 5             # Minimum consecutive years of dividend increases
PAYOUT_MAX = 0.80          # Payout ratio cap
PAYOUT_MIN = 0.0           # Exclude negative payout (negative earnings)
FCF_MIN = 0                # Free cash flow must be positive
MAX_STOCKS = 30
MIN_STOCKS = 10
DEFAULT_FREQUENCY = "annual"
DEFAULT_REBALANCE_MONTHS = [7]  # July (full prior year dividends + financials available)
...
def fetch_data_via_api(client, exchanges, rebalance_dates, verbose=False):
    """Fetch financial data and dividend calendar, load into DuckDB."""
    if exchanges:
    ...
    # 2. Dividend calendar - the core data for this strategy
    print("  Fetching dividend calendar...")
    div_sql = f"""
        SELECT d.symbol, d.date, d.adjDividend
        FROM dividend_calendar d
        WHERE d.adjDividend > 0
          AND d.symbol IN (SELECT DISTINCT symbol FROM profile {exchange_where})
    """
    count = query_parquet(client, div_sql, con, "div_calendar",
                          verbose=verbose, limit=2000000, timeout=600)
    print(f"    -> {count} dividend records")

    if count == 0:
        print("  No dividend data. Skipping.")
        con.close()
        return None
    ...
    # Precompute year-over-year changes
    con.execute("""
        CREATE TABLE div_yoy AS
        SELECT symbol, yr, total_div,
            LAG(total_div) OVER (PARTITION BY symbol ORDER BY yr) AS prev_div,
            CASE WHEN total_div > LAG(total_div) OVER (PARTITION BY symbol ORDER BY yr)
                 THEN 1 ELSE 0 END AS increased
        FROM div_annual
    """)
    # Remove rows without a prior year for comparison
    con.execute("DELETE FROM div_yoy WHERE prev_div IS NULL")

    yoy_count = con.execute("SELECT COUNT(*) FROM div_yoy").fetchone()[0]
    print(f"    -> {yoy_count} year-over-year comparisons")

    # 3. Financial data (for quality filters)
    queries = [
        ("ratios_cache", f"""
            SELECT symbol, dividendPayoutRatio,
                   dateEpoch as filing_epoch, period
            FROM financial_ratios
            WHERE period = 'FY'
              AND {sym_filter_sql}
        """, "financial ratios (payout)"),
        ("metrics_cache", f"""
    ...
    price_sql = f"""
        ...
        WHERE ({date_filter})
          AND (
            ...
            OR symbol IN (
                SELECT DISTINCT symbol FROM dividend_calendar
                WHERE adjDividend > 0
                {f"AND symbol IN (SELECT DISTINCT symbol FROM profile WHERE exchange IN ({ex_filter}))" if exchanges else ""}
            )
          )
    """
...
def screen_stocks(con, target_date, mktcap_min):
    """Screen for dividend growth stocks with quality filters.

    Computes current consecutive dividend increase streak for each symbol
    as of the year before target_date, then applies quality filters.
    ...
    rows = con.execute("""
        WITH last_break AS (
            ...
            FROM div_yoy
            WHERE increased = 0 AND yr <= ?
            GROUP BY symbol
        ),
        streak AS (
            SELECT d.symbol, COUNT(*) AS consecutive_years
            FROM div_yoy d
            LEFT JOIN last_break lb ON d.symbol = lb.symbol
            WHERE d.increased = 1
              AND d.yr <= ?
              AND (lb.break_yr IS NULL OR d.yr > lb.break_yr)
            GROUP BY d.symbol
            HAVING COUNT(*) >= ?
        ),
        fr AS (
        ...
        SELECT s.symbol, km.marketCap, s.consecutive_years
        FROM streak s
        JOIN fr ON s.symbol = fr.symbol AND fr.rn = 1
        JOIN km ON s.symbol = km.symbol AND km.rn = 1
        JOIN cf ON s.symbol = cf.symbol AND cf.rn = 1
    ...
    """, [max_year, max_year, MIN_STREAK,
          ...
          PAYOUT_MIN, PAYOUT_MAX, FCF_MIN, mktcap_min, MAX_STOCKS]).fetchall()
...
def run_backtest(con, rebalance_dates, mktcap_min, use_costs=True, verbose=False,
    ...
    """Run Dividend Growth backtest. Returns list of period result dicts."""
    results = []
...
def run_single(cr, exchanges, universe_name, frequency, use_costs,
    ...
    signal_desc = (f"DivGrowth streak >= {MIN_STREAK}yr, "
                   f"Payout < {PAYOUT_MAX*100:.0f}%, FCF > 0, "
                   f"MCap > {mktcap_label} local, top {MAX_STOCKS}")
    print_header("DIVIDEND GROWTH (ARISTOCRATS) BACKTEST", universe_name, exchanges, signal_desc)
    ...
    print(format_metrics(metrics, "DivGrowth", benchmark_name))
    ...
    if annual:
        ...
        print(f"\n  {'Year':<8} {'DivGrowth':>12} {bench_label:>10} {'Excess':>10}")
        print("  " + "-" * 42)
        for ar in annual:
...
def main():
    parser = argparse.ArgumentParser(description="Dividend Growth (Aristocrats) backtest")
    ...
    if args.cloud:
        ...
        result = run_backtest_cloud("dividend-growth", args_str=" ".join(cloud_args),
    ...
    if exchanges is None and universe_name in ("Global", "GLOBAL"):
        ...
        print("-" * 80)
        for uni, r in sorted(all_results.items(), key=lambda x: (x[1].get("portfolio") or {}).get("cagr") or -999, reverse=True):
