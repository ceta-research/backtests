#!/usr/bin/env python3
"""
Deleveraging Strategy Backtest
...
Academic basis: Deleveraging as a contrarian signal for balance sheet improvement.
Companies reducing leverage signal management discipline, improved cash flows,
and reduced financial distress risk.
...
Usage:
    ...
    python3 deleveraging/backtest.py --no-costs
...
MAX_SINGLE_RETURN = 2.0       # Cap individual stock returns at 200% (data quality)
MIN_ENTRY_PRICE = 1.0         # Skip stocks with entry price < $1 (price artifact)
...
def fetch_data_via_api(client, exchanges, rebalance_dates, verbose=False):
    ...
    # 2-3. Financial data (need both FY D/E and quality metrics)
    queries = [
        ("ratios_cache", f"""
...
def screen_stocks(con, target_date, mktcap_min):
    """Screen for deleveraging stocks.

    Finds stocks where D/E ratio dropped >10% year-over-year, company was profitable
    (ROE > 8%), and had meaningful debt to reduce (prior D/E > 0.1).
    ...
    rows = con.execute("""
        ...
        SELECT cf.symbol, km.marketCap,
            (cf.de_current - pf.de_prior) / pf.de_prior AS de_change
        FROM current_fy cf
        JOIN prior_fy pf ON cf.symbol = pf.symbol AND pf.rn = 1
        JOIN km ON cf.symbol = km.symbol AND km.rn = 1
        WHERE cf.rn = 1
          AND cf.filing_epoch != pf.prior_epoch
          AND pf.de_prior > ?
          AND cf.de_current >= ?
    ...
    """, [cutoff_epoch, prior_cutoff_epoch, cutoff_epoch,
          ...
          ROE_MIN, mktcap_min, MAX_STOCKS]).fetchall()
...
def run_backtest(con, rebalance_dates, mktcap_min, use_costs=True, verbose=False,
    ...
    """Run Deleveraging backtest. Returns list of period result dicts."""
    results = []
...
def run_single(cr, exchanges, universe_name, frequency, use_costs,
    ...
    signal_desc = (f"D/E YoY < {DE_CHANGE_THRESHOLD*100:.0f}%, prior D/E > {DE_PRIOR_MIN}, "
                   f"ROE > {ROE_MIN*100:.0f}%, MCap > {mktcap_label} local, top {MAX_STOCKS}")
    benchmark_symbol, benchmark_name = get_local_benchmark(exchanges)
    exec_model = "MOC (next-day close)" if offset_days == 1 else "same-bar close"
    print_header("DELEVERAGING BACKTEST", universe_name, exchanges, signal_desc)
    ...
    # Start from 2000 so we have prior-year data for 2001 rebalances
    ...
    print(format_metrics(metrics, "Deleveraging", benchmark_name))
    ...
    if annual:
        bench_col = benchmark_name[:9]
        print(f"\n  {'Year':<8} {'Deleverag':>10} {bench_col:>10} {'Excess':>10}")
        print("  " + "-" * 42)
        for ar in annual:
...
def main():
    parser = argparse.ArgumentParser(description="Deleveraging strategy backtest")
    ...
    if args.cloud:
        ...
        result = run_backtest_cloud("deleveraging", args_str=" ".join(cloud_args),
