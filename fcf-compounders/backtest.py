#!/usr/bin/env python3
"""
FCF Compounders Backtest

Annual rebalancing (July), equal weight, top 30 by highest ROIC among FCF compounders.
Fetches data via configurable provider, caches in DuckDB, runs locally.

Signal: FCF grew in >= 4 of last 5 FY years, all FCF positive,
        ROIC > 15%, Operating Margin > 15%, Market Cap > local-currency threshold
Portfolio: Top 30 by highest ROIC, equal weight. Cash if < 10 qualify.
Rebalancing: Annual (July), 2000-2025.

Academic reference: Mohanram, P. (2005) "Separating Winners from Losers among Low
Book-to-Market Stocks using Financial Statement Analysis", Review of Accounting
Studies 10(2-3), 133-170. Combining growth signals with quality metrics.

Usage:
    python3 fcf-compounders/backtest.py
    python3 fcf-compounders/backtest.py --preset india
    python3 fcf-compounders/backtest.py --global --output results/exchange_comparison.json --verbose
    python3 fcf-compounders/backtest.py --no-costs
...
# --- Signal parameters ---
FCF_GROWTH_YEARS_MIN = 4   # At least 4 out of 5 years with FCF growth
FCF_LOOKBACK_YEARS = 5     # 5-year consistency window
ROIC_MIN = 0.15            # Return on invested capital > 15%
OPM_MIN = 0.15             # Operating profit margin > 15%
MAX_STOCKS = 30
MIN_STOCKS = 10
...
def fetch_data_via_api(client, exchanges, rebalance_dates, verbose=False):
    ...
    Populates tables:
        ...
        cashflow_cache(symbol, freeCashFlow, filing_epoch, period)
        metrics_cache(symbol, returnOnInvestedCapital, marketCap, filing_epoch, period)
    ...
    # 2. Cash flow statement (need 6+ FY filings for 5-year growth window)
    queries = [
        ("cashflow_cache", f"""
            ...
            WHERE period = 'FY'
              ...
              AND {sym_filter_sql}
        """, "cash flow statement (FCF history)"),
        ("metrics_cache", f"""
            SELECT symbol, returnOnInvestedCapital, marketCap,
                   dateEpoch as filing_epoch, period
            FROM key_metrics
            WHERE period = 'FY'
              AND returnOnInvestedCapital IS NOT NULL
              AND {sym_filter_sql}
        """, "key metrics (ROIC, market cap)"),
        ("ratios_cache", f"""
    ...
    price_sql = f"""
        ...
        WHERE ({date_filter})
          AND (
            ...
            OR symbol IN (
                SELECT DISTINCT symbol FROM cash_flow_statement WHERE period = 'FY'
...
def screen_stocks(con, target_date, mktcap_min):
    """Screen for FCF compounders: consistent multi-year FCF growth + quality.

    Computes 5-year FCF growth consistency score from FY cash flow statements.
    ...
    rows = con.execute("""
        ...
        -- Get the most recent 6 filings (need 6 to compute 5 YoY changes)
        recent_fcf AS (
            SELECT symbol, freeCashFlow, filing_epoch, rn
            FROM ranked_fcf
            WHERE rn <= 6
        ),
        -- Compute YoY growth for each pair of consecutive filings
        fcf_growth AS (
            SELECT
                curr.symbol,
                curr.freeCashFlow AS curr_fcf,
                prev.freeCashFlow AS prev_fcf,
                curr.rn
            FROM recent_fcf curr
            JOIN recent_fcf prev ON curr.symbol = prev.symbol AND curr.rn = prev.rn - 1
        ),
        -- Aggregate: count growth years, check all positive
        fcf_stats AS (
            SELECT
                symbol,
                COUNT(*) AS total_pairs,
                SUM(CASE WHEN curr_fcf > prev_fcf AND prev_fcf > 0 THEN 1 ELSE 0 END) AS growth_years,
                MIN(curr_fcf) AS min_fcf,
        ...
        SELECT fs.symbol, km.marketCap, fs.growth_years,
               km.returnOnInvestedCapital AS roic
        FROM fcf_stats fs
        JOIN km ON fs.symbol = km.symbol AND km.rn = 1
        JOIN fr ON fs.symbol = fr.symbol AND fr.rn = 1
        WHERE fs.growth_years >= ?
          AND fs.min_fcf > 0
...
def run_backtest(con, rebalance_dates, mktcap_min, use_costs=True, verbose=False,
    ...
    """Run FCF Compounders backtest. Returns list of period result dicts."""
    results = []
...
def run_single(cr, exchanges, universe_name, frequency, use_costs,
    ...
    signal_desc = (f"FCF grew {FCF_GROWTH_YEARS_MIN}+/5yr, all FCF>0, "
                   f"ROIC > {ROIC_MIN*100:.0f}%, OPM > {OPM_MIN*100:.0f}%, "
                   f"MCap > {mktcap_label} local, top {MAX_STOCKS} by ROIC")
    ...
    print_header("FCF COMPOUNDERS BACKTEST", universe_name, exchanges, signal_desc)
    ...
    print(format_metrics(metrics, "FCF Compounders", benchmark_name))
    ...
    if annual:
        bench_header = benchmark_name[:10] if len(benchmark_name) > 10 else benchmark_name
        print(f"\n  {'Year':<8} {'FCFCompound':>12} {bench_header:>10} {'Excess':>10}")
        print("  " + "-" * 42)
        for ar in annual:
...
def main():
    parser = argparse.ArgumentParser(description="FCF Compounders backtest")
    ...
    if args.cloud:
        ...
        result = run_backtest_cloud("fcf-compounders", args_str=" ".join(cloud_args),
    ...
    if exchanges is None and universe_name in ("Global", "GLOBAL"):
        ...
        for preset_name, preset_exchanges in presets_to_run:
            if preset_name in EXCLUDED_PRESETS:
                print(f"\nSkipping {preset_name} - excluded (data quality)")
                continue
