"""Generate all Sector Rotation charts for blog posts from exchange_comparison.json.

Run from backtests/ directory:
    python3 sector-rotation/generate_charts.py

Charts are saved to backtests/sector-rotation/charts/.
...
EXCHANGE_LABELS = {
    "NYSE_NASDAQ_AMEX": "Sector Rotation US",
    "BSE_NSE": "Sector Rotation India",
    "XETRA": "Sector Rotation Germany",
    "LSE": "Sector Rotation UK",
    "TSX": "Sector Rotation Canada",
    "KSC": "Sector Rotation Korea",
    "ASX": "Sector Rotation Australia",
    "TAI_TWO": "Sector Rotation Taiwan",
    "SAO": "Sector Rotation Brazil",
    "HKSE": "Sector Rotation HK",
...
EXCHANGE_UNIVERSE_LABELS = {
    ...
    "ASX": "ASX (returns in AUD)",
    "TAI_TWO": "TAI + TWO (returns in TWD)",
    "SAO": "SAO (returns in BRL)",
...
def chart_comparison_cagr(filename):
    ...
    spy_cagr = data.get("NYSE_NASDAQ_AMEX", {}).get("spy", {}).get("cagr", 10.84)
    ...
    ax.set_title("Sector Mean Reversion CAGR by Exchange (2000-2025)",
    ...
    fig.text(0.5, -0.02,
             "Data: Ceta Research | Bottom 2 sectors by 12M trailing return, quarterly rebalance",
             ha="center", fontsize=8, color="#7f8c8d")
...
def chart_comparison_drawdown(filename):
    ...
    spy_dd = data.get("NYSE_NASDAQ_AMEX", {}).get("spy", {}).get("max_drawdown", -50)
    ...
    ax.set_title("Sector Mean Reversion: Max Drawdown by Exchange (2000-2025)",
                 fontsize=13, fontweight="bold", pad=15)
    ax.legend(fontsize=10)
    ax.grid(True, alpha=0.3, axis="x", linestyle="--")
...
print("\nNext step: Move charts to ts-content-creator/content/_current/reversion-04-sector-rotation/blogs/{region}/")
