#!/usr/bin/env python3
"""
Sector Mean Reversion — Current Screen

Shows current sector rankings by trailing 12-month return and the stocks
currently in the worst-performing sectors. Run this to see what the strategy
would buy today.

Usage:
    python3 sector-rotation/screen.py                    # US (NYSE+NASDAQ+AMEX)
    python3 sector-rotation/screen.py --preset india     # India
    python3 sector-rotation/screen.py --n-worst 3        # Bottom 3 sectors

Output:
    - Sector ranking table (worst to best by 12-month return)
    - Stocks in the selected bottom N sectors
...
def run_screen(cr, exchanges, universe_name, n_worst=N_WORST_SECTORS, verbose=False):
    ...
    print(f"\n{'=' * 60}")
    print(f"  SECTOR MEAN REVERSION — CURRENT SCREEN")
    print(f"  Universe: {universe_name}")
    print(f"  Date: {date.today().isoformat()}")
    print(f"  Signal: Bottom {n_worst} sectors by 12-month trailing return")
...
ORDER BY avg_return_12m_pct ASC
    ...
    for row in sector_rows:
        rank = row.get("rank_worst", "?")
        sector = row.get("sector", "?")
        ret = row.get("avg_return_12m_pct", 0)
        n = row.get("n_stocks", 0)
        marker = " <<" if rank <= n_worst else ""
        print(f"{rank:<6} {sector:<30} {ret:>+11.1f}% {n:>10}{marker}")
        if rank <= n_worst:
            worst_sectors.append(sector)

    if not worst_sectors:
        print("\nNot enough sectors qualify.")
        return

    print(f"\n  Selected (bottom {n_worst}): {', '.join(worst_sectors)}")

    # Query: stocks in worst sectors
    sectors_str = ", ".join(f"'{s}'" for s in worst_sectors)
    stocks_sql = f"""
WITH prices AS (
    SELECT e.symbol, e.adjClose, CAST(e.date AS DATE) AS trade_date
    FROM stock_eod e
...
WHERE ya.old_price > 0 AND r.recent_price > 0
  AND (r.recent_price / ya.old_price - 1) BETWEEN -0.99 AND 5.0
ORDER BY pr.sector ASC, return_12m_pct ASC
LIMIT 100
    """
...
def main():
    parser = argparse.ArgumentParser(description="Sector Mean Reversion current screen")
    add_common_args(parser)
    parser.add_argument("--n-worst", type=int, default=N_WORST_SECTORS,
                        help=f"Number of worst sectors to show (default {N_WORST_SECTORS})")
    ...
    if args.cloud:
        ...
        result = run_backtest_cloud("sector-rotation/screen",
    ...
    run_screen(cr, exchanges, universe_name, n_worst=args.n_worst, verbose=args.verbose)
