...
def run_backtest(con, rebalance_dates, mktcap_min, use_costs=True, verbose=False,
    ...
    for i in range(len(rebalance_dates) - 1):
        ...
        buyable = entry_buyable_prices(symbols, entry_prices, min_entry_price=0.0)
        if buyable < MIN_STOCKS:
            ...
            if verbose:
                print(f"    {entry_date}: only {buyable} of {len(symbols)} screened "
...
def main():
    ...
    print(format_metrics(metrics, "Low P/E", benchmark_name))
    ...
    if annual:
        ...
        print(f"\n  {'Year':<8} {'Low P/E':>10} {bench_label:>10} {'Excess':>10}")
        print("  " + "-" * 40)
        for ar in annual:
