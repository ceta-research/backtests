...
def fetch_data_via_api(client, exchanges, rebalance_dates, verbose=False):
    ...
    # Benchmark symbols: SPY plus local index for the exchange set
    bench_symbols = {"'SPY'"}
    if exchanges:
        for ex in exchanges:
...
def run_single(cr, exchanges, universe_name, frequency, use_costs,
    ...
    print(format_metrics(metrics, "Earnings Consistency", benchmark_name))
    ...
    if annual:
        ...
        print(f"\n  {'Year':<8} {'EarnCons':>10} {bench_col:>10} {'Excess':>10}")
        print("  " + "-" * 40)
        for ar in annual:
