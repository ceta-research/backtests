#!/usr/bin/env python3
"""
Free Cash Flow Growth Backtest

Annual rebalancing (July), equal weight, top 30 by highest FCF growth.
Fetches data via configurable provider, caches in DuckDB, runs locally.

Signal: FCF growth YoY > 15%, OCF growth YoY > 0 (capex guard), ROE > 10%,
        D/E < 1.5, MCap > local-currency threshold
Portfolio: Top 30 by FCF growth DESC, equal weight. Cash if < 10 qualify.
Rebalancing: Annual (July), 2000-2025.

Academic reference: Sloan, R. (1996). "Do Stock Prices Fully Reflect
Information in Accruals and Cash Flows About Future Earnings?"
The Accounting Review, 71(3), 289-315.
(Accrual anomaly: cash-backed earnings outperform accrual-heavy earnings.)
...
Usage:
    ...
    python3 fcf-growth/backtest.py --preset india
...
# --- Signal parameters ---
FCF_GROWTH_MIN = 0.15      # FCF must grow at least 15% YoY
OCF_GROWTH_MIN = 0.0       # OCF must also be growing (capex-cut guard)
ROE_MIN = 0.10             # Return on equity > 10%
DE_MAX = 1.5               # Debt-to-equity < 1.5
MAX_STOCKS = 30
MIN_STOCKS = 10
DEFAULT_FREQUENCY = "annual"
DEFAULT_REBALANCE_MONTHS = [7]   # July (FY filings available by then with 45-day lag)
...
def fetch_data_via_api(client, exchanges, rebalance_dates, verbose=False, domicile=False):
    ...
    Populates tables:
        fcf_cache(symbol, freeCashFlow, operatingCashFlow, filing_epoch, period)
    ...
    # 2. Cash flow statement (need two consecutive FY filings for YoY growth)
    queries = [
        ("fcf_cache", f"""
...
def screen_stocks(con, target_date, mktcap_min):
    """Screen for high FCF growth quality stocks.

    Computes YoY FCF and OCF growth from two consecutive FY cash flow filings.
    Returns list of (symbol, market_cap) tuples sorted by FCF growth DESC.
    """
    ...
    rows = con.execute("""
        ...
        FROM cf_current cc
        JOIN cf_prior cp ON cc.symbol = cp.symbol AND cp.rn = 1
        JOIN km ON cc.symbol = km.symbol AND km.rn = 1
        JOIN fr ON cc.symbol = fr.symbol AND fr.rn = 1
        WHERE cc.rn = 1
          AND cc.filing_epoch != cp.filing_epoch
          AND cc.freeCashFlow > 0
          AND (cc.freeCashFlow - cp.fcf_prior) / NULLIF(ABS(cp.fcf_prior), 0) > ?
          AND (cc.operatingCashFlow - cp.ocf_prior) / NULLIF(ABS(cp.ocf_prior), 0) > ?
    ...
    """, [cutoff_epoch, prior_cutoff_epoch, cutoff_epoch, cutoff_epoch,
          FCF_GROWTH_MIN, OCF_GROWTH_MIN, ROE_MIN, DE_MAX,
...
def run_backtest(con, rebalance_dates, mktcap_min, use_costs=True, verbose=False,
    ...
    """Run FCF Growth backtest. Returns list of period result dicts."""
    results = []
    ...
    for i in range(len(rebalance_dates) - 1):
        ...
        # stock's year as a diversified portfolio return. XETRA 2001 did exactly
        # that: 1 stock, -49.4%, which alone set the reported max drawdown.
...
def run_single(cr, exchanges, universe_name, frequency, use_costs,
    ...
    signal_desc = (f"FCF growth > {FCF_GROWTH_MIN*100:.0f}%, OCF growth > {OCF_GROWTH_MIN*100:.0f}%, "
                   f"ROE > {ROE_MIN*100:.0f}%, D/E < {DE_MAX}, MCap > {mktcap_label} local, top {MAX_STOCKS} by FCF growth")
    print_header("FCF GROWTH BACKTEST", universe_name, exchanges, signal_desc)
    ...
    exec_model = "MOC (next-day close)" if offset_days == 1 else "same-bar (current close)"
    ...
    print(format_metrics(metrics, "FCF Growth", benchmark_name))
    ...
    if annual:
        print(f"\n  {'Year':<8} {'FCFGrowth':>12} {benchmark_name:>12} {'Excess':>10}")
        print("  " + "-" * 46)
        for ar in annual:
            print(f"  {ar['year']:<8} {ar['portfolio']*100:>11.1f}% {ar['benchmark']*100:>11.1f}% "
...
def main():
    parser = argparse.ArgumentParser(description="Free Cash Flow Growth backtest")
    ...
    if args.cloud:
        ...
        result = run_backtest_cloud("fcf-growth", args_str=" ".join(cloud_args),
    ...
    if exchanges is None and universe_name in ("Global", "GLOBAL"):
        ...
        presets_to_run = [
            ...
            # SES excluded: avg 8.4 stocks/period when invested (below MIN_STOCKS quality threshold)
            # SET excluded: strategy fails (<1% CAGR over 25yr), 5 early cash years, -61% MaxDD
        ]
