#!/usr/bin/env python3
"""
Run Dogs of the Dow backtest across all eligible exchanges.
Aggregates results into a single exchange_comparison.json file.

US uses true Dow 30 membership. Other exchanges use top 30 by market cap.

Usage:
    python3 dogs-of-dow/run_all_exchanges.py --verbose
    python3 dogs-of-dow/run_all_exchanges.py --output dogs-of-dow/results/exchange_comparison.json
"""
...
from backtest import (
    ...
    DOGS_COUNT, BLUECHIP_COUNT, MIN_STOCKS, DEFAULT_FREQUENCY,
)
...
def main():
    import argparse
    parser = argparse.ArgumentParser(description="Run Dogs of the Dow backtest on all exchanges")
    parser.add_argument("--output", type=str, default="dogs-of-dow/results/exchange_comparison.json")
    ...
    for config in EXCHANGE_CONFIGS:
        ...
        print(f"\n{'='*65}")
        print(f"  {name} ({', '.join(exchanges)}) — {strategy_label}")
