...
def run_single(cr, exchanges, universe_name, frequency, use_costs,
    ...
    # Count cash/invested over `valid` (periods inside the benchmark's data window),
    # not all `results`. For markets whose local index starts after 2000 (e.g. OSL/Norway
    # from 2013), the pre-coverage rebalance dates are not measured periods, so counting
    # them as cash makes cash_periods exceed n_periods and invested_periods go negative.
