#!/usr/bin/env python3
"""
Earnings Yield Backtest

Quarterly rebalancing, equal weight, top 50 by highest earnings yield.
Fetches data via configurable provider, caches in DuckDB, runs locally.

Signal: earningsYield > 0, ROE > 12%, D/E < 1.5, IC > 3,
        Market Cap > local currency threshold, top 50 by EY DESC.
Portfolio: Equal weight. Cash if < 10 qualify.
Rebalancing: Quarterly (Jan/Apr/Jul/Oct), 2000-2025.

Earnings yield = E/P = 1 / P/E. A stock trading at P/E=10 has 10% earnings yield.
Higher EY = more earnings per dollar of price = cheaper on an earnings basis.

Academic reference:
  Loughran & Wellman (2011) "New evidence on the relation between the enterprise
  multiple and average stock returns". Also: Fama & French (1992).
...
Usage:
    ...
    python3 earnings-yield/backtest.py --preset india

    # Backtest all exchanges
    python3 earnings-yield/backtest.py --global --output results/exchange_comparison.json --verbose

    # Without transaction costs
    python3 earnings-yield/backtest.py --no-costs
...
# --- Signal parameters ---
EY_MIN = 0.0           # Positive earnings yield (profitable companies only)
ROE_MIN = 0.12         # Return on equity > 12%
DE_MAX = 1.5           # Debt/equity < 1.5
IC_MIN = 3.0           # Interest coverage > 3x (can service debt)
MAX_STOCKS = 50        # Top 50 by highest earnings yield
MIN_STOCKS = 10        # Hold cash if fewer than 10 qualify
DEFAULT_FREQUENCY = "quarterly"
MAX_SINGLE_RETURN = 2.0   # Cap individual returns at 200% (data quality guard)
MIN_ENTRY_PRICE = 1.0     # Skip stocks with price < $1 (price data artifact)
...
def fetch_data_via_api(client, exchanges, rebalance_dates, verbose=False):
    ...
    Populates tables:
        ...
        ratios_cache(symbol, debtToEquityRatio, interestCoverageRatio, filing_epoch, period)
    ...
    queries = [
        ...
        ("ratios_cache", f"""
            SELECT symbol, debtToEquityRatio, interestCoverageRatio,
                   dateEpoch as filing_epoch, period
            ...
            WHERE period = 'FY'
              ...
              AND {sym_filter_sql}
        """, "financial ratios (D/E, interest coverage)"),
    ]
    ...
    # Build benchmark symbol list: SPY + local index for each exchange
    bench_symbols = {"'SPY'"}
    if exchanges:
        for ex in exchanges:
...
def screen_stocks(con, target_date, mktcap_min):
    """Screen for high earnings yield stocks.

    Uses most recent FY filing available at least 45 days before target date.
    Returns list of (symbol, market_cap) tuples sorted by earningsYield DESC.
    """
    ...
    rows = con.execute("""
        ...
        r AS (
            SELECT symbol, debtToEquityRatio, interestCoverageRatio, filing_epoch,
                ROW_NUMBER() OVER (PARTITION BY symbol ORDER BY filing_epoch DESC) AS rn
        ...
        WHERE m.rn = 1
          ...
          AND m.marketCap > ?
        ORDER BY m.earningsYield DESC
        LIMIT ?
    """, [cutoff_epoch, cutoff_epoch,
          EY_MIN, ROE_MIN, DE_MAX, IC_MIN, mktcap_min, MAX_STOCKS]).fetchall()
...
def run_backtest(con, rebalance_dates, mktcap_min, use_costs=True, verbose=False,
    ...
    """Run Earnings Yield backtest. Returns list of period result dicts."""
    results = []
...
def run_single(cr, exchanges, universe_name, frequency, use_costs,
    ...
    signal_desc = (f"EY > {EY_MIN*100:.0f}%, ROE > {ROE_MIN*100:.0f}%, "
                   f"D/E < {DE_MAX}, IC > {IC_MIN:.0f}x, "
                   f"MCap > {mktcap_label} local, top {MAX_STOCKS} by EY")
    print_header("EARNINGS YIELD BACKTEST", universe_name, exchanges, signal_desc)
    ...
    print(format_metrics(metrics, "Earnings Yield", benchmark_name))
    ...
    if annual:
        bench_col = f"{benchmark_name[:8]:>10}"
        print(f"\n  {'Year':<8} {'EarningsYld':>12} {bench_col} {'Excess':>10}")
        print("  " + "-" * 42)
        for ar in annual:
...
def main():
    parser = argparse.ArgumentParser(description="Earnings Yield backtest")
    ...
    if args.cloud:
        ...
        result = run_backtest_cloud("earnings-yield", args_str=" ".join(cloud_args),
    ...
    if exchanges is None and universe_name in ("Global", "GLOBAL"):
        ...
        all_results = {}
        # Exchanges with earningsYield + IC data and enough universe depth
