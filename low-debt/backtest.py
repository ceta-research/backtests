#!/usr/bin/env python3
"""
Low Debt Quality Backtest

Annual rebalancing (July), equal weight, all qualifying stocks.
Fetches data via configurable provider, caches in DuckDB, runs locally.

Signal: D/E < 0.5 (FY) + Piotroski F-Score >= 7 + MCap > local-currency threshold
Portfolio: All qualifying stocks, equal weight. Cash if < 10 qualify.
...
Academic references:
  ...
  - Graham & Dodd (1934) "Security Analysis", McGraw-Hill.

Usage:
    # Backtest US stocks (default)
    python3 low-debt/backtest.py

    # Backtest Indian stocks
    python3 low-debt/backtest.py --preset india
...
PIOTROSKI_MIN = 7          # Piotroski F-Score >= 7 (out of 9)
...
DEFAULT_REBALANCE_MONTHS = [7]  # July — annual FY filings available by then (45-day lag)
...
def fetch_data_via_api(client, exchanges, rebalance_dates, verbose=False):
    ...
    # 2-6: Financial data (via parquet for bulk transfer efficiency)
    queries = [
        ("income_cache", f"""
    ...
    # 7. Prices (only at rebalance dates + 1 day for MOC execution)
    print("  Fetching prices...")
    date_conditions = []
    for d in rebalance_dates:
        end = d + timedelta(days=12)  # +12 to cover offset_days=1 + window_days=10
        date_conditions.append(f"(date >= '{d.isoformat()}' AND date <= '{end.isoformat()}')")
    date_filter = " OR ".join(date_conditions)

    # Always fetch SPY + any local benchmark for the exchange(s)
    bench_symbols = {"'SPY'"}
    if exchanges:
        for ex in exchanges:
...
def screen_stocks(con, target_date, mktcap_min):
    """Screen for low D/E + high Piotroski stocks.

    Computes Piotroski F-Score from two consecutive FY filings.
    D/E < 0.5 from financial_ratios FY.
    ...
    rows = con.execute("""
        ...
        scored AS (
            SELECT
                ...
                -- F3: ROA improving (net income / total assets)
                ...
                -- F4: Accrual quality (OCF > net income = earnings backed by cash)
                ...
                CASE WHEN bc.totalAssets > 0 AND bp.totalAssets > 0
                     AND (COALESCE(bc.longTermDebt, 0) / bc.totalAssets) <
                         (COALESCE(bp.longTermDebt, 0) / bp.totalAssets) THEN 1 ELSE 0 END AS f5_leverage,
                ...
                CASE WHEN bc.totalCurrentAssets > 0 AND bc.totalCurrentLiabilities > 0
                     ...
                     AND (bc.totalCurrentAssets / bc.totalCurrentLiabilities) >
                         (bp.totalCurrentAssets / bp.totalCurrentLiabilities) THEN 1 ELSE 0 END AS f6_liquidity,
                -- F7: No equity dilution (shares not increased)
    ...
    """, [
        ...
        cutoff_epoch, stale_cutoff_epoch,        # bal_curr
        prior_cutoff_epoch, stale_cutoff_epoch,  # bal_prev
        cutoff_epoch, stale_cutoff_epoch,        # cf_curr
...
def run_backtest(con, rebalance_dates, mktcap_min, use_costs=True, verbose=False,
    ...
    """Run Low Debt Quality backtest. Returns list of period result dicts."""
    results = []
...
def run_single(cr, exchanges, universe_name, frequency, use_costs,
    ...
    signal_desc = (f"D/E < {DE_MAX}, Piotroski >= {PIOTROSKI_MIN}, MCap > {mktcap_label} local")
    benchmark_symbol, benchmark_name = get_local_benchmark(exchanges)
    exec_model = "same-day" if offset_days == 0 else "next-day (MOC)"
    print_header("LOW DEBT QUALITY BACKTEST", universe_name, exchanges, signal_desc)
    ...
    print(format_metrics(metrics, "Low Debt Quality", benchmark_name))
    ...
    if annual:
        bench_col = benchmark_name[:8]
        print(f"\n  {'Year':<8} {'LowDebt':>12} {bench_col:>10} {'Excess':>10}")
        print("  " + "-" * 42)
        for ar in annual:
...
def main():
    parser = argparse.ArgumentParser(description="Low Debt Quality backtest")
    ...
    if args.cloud:
        ...
        result = run_backtest_cloud("low-debt", args_str=" ".join(cloud_args),
