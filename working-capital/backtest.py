#!/usr/bin/env python3
"""
Working Capital Efficiency Backtest

Annual rebalancing (June), equal weight, top 30 by lowest WC/Revenue.
Fetches data via configurable provider, caches in DuckDB, runs locally.

Signal: WC/Revenue < 0.50, positive revenue growth, ROE > 8%, OPM > 10%, MCap > $500M
Portfolio: Top 30 by lowest WC/Revenue, equal weight. Cash if < 10 qualify.
Rebalancing: Annual (June), 2000-2025.

Academic reference: Sloan, R. (1996). "Do Stock Prices Fully Reflect Information
in Accruals and Cash Flows About Future Earnings?" The Accounting Review, 71(3), 289-315.
Also: Hirshleifer, Hou, Teoh & Zhang (2004), Richardson et al. (2005).

Usage:
    # Backtest US stocks (default)
    python3 working-capital/backtest.py

    # Backtest Indian stocks
    python3 working-capital/backtest.py --preset india

    # Backtest all exchanges (global loop)
...
# --- Signal parameters ---
WC_RATIO_MAX = 0.50        # Buy stocks with WC/Revenue < 50%
...
DEFAULT_REBALANCE_MONTHS = [6]  # June (annual FY filings available by then)
...
def fetch_data_via_api(client, exchanges, rebalance_dates, verbose=False):
    ...
    Populates tables:
        ...
        bs_cache(symbol, workingCapital, filing_epoch, period)
        inc_cache(symbol, revenue, filing_epoch, period)
    ...
    # 2. Balance sheet (compute WC from totalCurrentAssets - totalCurrentLiabilities)
    # Note: balance_sheet API table uses totalCurrentAssets/totalCurrentLiabilities,
    # not a pre-computed workingCapital column.
    queries = [
        ("bs_cache", f"""
            SELECT symbol,
            ...
            WHERE period = 'FY'
              ...
              AND {sym_filter_sql}
        """, "balance sheet (working capital = currentAssets - currentLiabilities)"),
        ("inc_cache", f"""
...
def screen_stocks(con, target_date, mktcap_min):
    """Screen for low WC/Revenue quality stocks.

    Returns list of (symbol, market_cap) tuples sorted by WC/Revenue ASC.
    ...
    rows = con.execute("""
        ...
        SELECT bs.symbol, km.marketCap,
            bs.workingCapital / ic.revenue AS wc_ratio
        FROM bs
        JOIN inc_current ic ON bs.symbol = ic.symbol AND ic.rn = 1
        JOIN inc_prior ip ON bs.symbol = ip.symbol AND ip.rn = 1
        JOIN km ON bs.symbol = km.symbol AND km.rn = 1
        JOIN fr ON bs.symbol = fr.symbol AND fr.rn = 1
        WHERE bs.rn = 1
          AND ic.revenue > 0
          AND bs.workingCapital > 0
          AND bs.workingCapital / ic.revenue < ?
...
def run_backtest(con, rebalance_dates, mktcap_min, use_costs=True, verbose=False,
    ...
    """Run Working Capital Efficiency backtest. Returns list of period result dicts."""
    results = []
...
def run_single(cr, exchanges, universe_name, frequency, use_costs,
    ...
    exec_model = "MOC (next-day close)" if offset_days == 1 else "same-day close"

    mktcap_label = f"{mktcap_min/1e9:.0f}B" if mktcap_min >= 1e9 else f"{mktcap_min/1e6:.0f}M"
    signal_desc = (f"WC/Rev < {WC_RATIO_MAX*100:.0f}%, rev growth > 0, "
                   f"ROE > {ROE_MIN*100:.0f}%, OPM > {OPM_MIN*100:.0f}%, "
                   f"MCap > {mktcap_label} local, top {MAX_STOCKS}")
    print_header("WORKING CAPITAL EFFICIENCY BACKTEST", universe_name, exchanges, signal_desc)
    ...
    print(format_metrics(metrics, "Working Capital", benchmark_name))
    ...
    if annual:
        ...
        print(f"\n  {'Year':<8} {'WorkingCap':>12} {bench_label:>10} {'Excess':>10}")
        print("  " + "-" * 42)
        for ar in annual:
...
def main():
    parser = argparse.ArgumentParser(description="Working Capital Efficiency backtest")
    ...
    if args.cloud:
        ...
        result = run_backtest_cloud("working-capital", args_str=" ".join(cloud_args),
