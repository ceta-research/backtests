...
def fetch_data_via_api(client, exchanges, rebalance_dates, verbose=False, domicile=False):
    ...
    for d in rebalance_dates:
        end = d + timedelta(days=12)  # +2 for MOC offset
...
def build_output(metrics, annual, valid, results, universe_name, frequency, periods_per_year,
                 cash_periods, avg_stocks, benchmark_symbol=None, benchmark_name=None):
...
def run_single(cr, exchanges, universe_name, frequency, use_costs,
    ...
    if domicile:
        print("  Universe: DOMICILE-FILTERED (home-country companies only)")
    print("=" * 65)
    ...
    print(format_metrics(metrics, "EV/EBITDA Sector-Relative", benchmark_name))
    ...
    if annual:
        print(f"\n  {'Year':<8} {'EV/EBITDA Rel':>15} {benchmark_name[:10]:>10} {'Excess':>10}")
        print("  " + "-" * 46)
        for ar in annual:
...
def main():
    ...
    parser.add_argument("--domicile-filter", action="store_true",
                        help="EXPERIMENTAL: restrict the universe to companies domiciled "
                             "in the exchange's home country (strips foreign secondary listings)")
    ...
    if exchanges is None and universe_name in ("Global", "GLOBAL"):
        ...
        print(f"  Execution: {'same-day' if offset_days == 0 else 'next-day (MOC)'}")
        print("=" * 65)
