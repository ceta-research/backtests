#!/usr/bin/env python3
"""
Industry Leader Backtest

Annual rebalancing (July), equal weight, top 3 companies by revenue in
each industry showing >= 5% YoY aggregate revenue growth.

Signal: Industry avg revenue growth >= 5% YoY, pick top 3 by revenue per
        qualifying industry, market cap > exchange threshold (standard)
Portfolio: Top 3 per qualifying industry, equal weight. Cash if < 10 qualify.
Rebalancing: Annual (July), 2001-2025 (need 1 prior year for growth calc).
...
Usage:
    ...
    python3 industry-leader/backtest.py --preset india

    # Backtest all exchanges (loop)
    python3 industry-leader/backtest.py --global --output results/exchange_comparison.json --verbose

    # Without transaction costs
    python3 industry-leader/backtest.py --no-costs
...
# --- Signal parameters ---
INDUSTRY_GROWTH_MIN = 0.05   # Industry avg revenue must grow >= 5% YoY
MIN_INDUSTRY_SIZE = 3        # Industry needs >= 3 qualifying companies
LEADERS_PER_INDUSTRY = 3     # Pick top N companies by revenue per industry
MIN_STOCKS = 10              # Hold cash if total portfolio < 10 stocks
MAX_STOCKS = 300             # Cap to prevent excessive tracking to benchmark
DEFAULT_FREQUENCY = "annual"
DEFAULT_REBALANCE_MONTHS = [7]  # July (after most FY filings are available)
MAX_SINGLE_RETURN = 2.0      # Cap individual stock returns at 200%
MIN_ENTRY_PRICE = 1.0        # Skip stocks with entry price < $1


def fetch_data_via_api(client, exchanges, rebalance_dates, verbose=False, offset_days=1):
    """Fetch financial data and load into DuckDB.

    Populates tables:
        income_cache(symbol VARCHAR, industry VARCHAR, revenue DOUBLE, filing_epoch BIGINT)
        metrics_cache(symbol VARCHAR, marketCap DOUBLE, filing_epoch BIGINT)
        prices_cache(symbol VARCHAR, trade_epoch BIGINT, adjClose DOUBLE)
    ...
    if exchanges:
        ...
        income_exchange_filter = f"AND p.exchange IN ({ex_filter})"
        sym_filter_sql = f"symbol IN (SELECT DISTINCT symbol FROM profile WHERE exchange IN ({ex_filter}))"
    else:
        exchange_where = "WHERE 1=1"
        income_exchange_filter = ""
        sym_filter_sql = "1=1"
    ...
    profile_sql = f"SELECT DISTINCT p.symbol, p.exchange FROM profile p {exchange_where}"
    ...
    # 2. Income statement with industry (FY only, revenue > 0)
    # Join profile for industry at fetch time (industry is a static attribute)
    print("  Fetching income statement (FY revenue + industry)...")
    income_sql = f"""
        SELECT i.symbol, p.industry, i.revenue, i.dateEpoch AS filing_epoch
    ...
    if count == 0:
        print("  No income data found. Skipping.")
        con.close()
        return None

    # 3. Key metrics (FY) for market cap filter
    print("  Fetching key metrics (FY market cap)...")
    metrics_sql = f"""
    ...
    # 4. Prices (around rebalance dates only)
    ...
    for d in rebalance_dates:
        end = d + timedelta(days=10 + max(0, offset_days) + 2)
...
def screen_stocks(con, target_date, mktcap_min):
    """Screen for industry leaders in growing industries.
    ...
    rows = con.execute("""
        ...
        company_growth AS (
            SELECT c.symbol, c.industry, c.revenue, p.prior_revenue,
                km.marketCap,
                (c.revenue - p.prior_revenue) / p.prior_revenue AS rev_growth
            FROM rev_current c
            JOIN rev_prior p ON c.symbol = p.symbol AND p.rn = 1
            JOIN km ON c.symbol = km.symbol AND km.rn = 1
            WHERE c.rn = 1
              AND p.prior_revenue > 0
              AND c.filing_epoch != p.prior_epoch
              AND km.marketCap >= ?
        ),
        industry_agg AS (
            SELECT industry,
    ...
    """, [cutoff_epoch, prior_cutoff_epoch, cutoff_epoch,
          mktcap_min,
          MIN_INDUSTRY_SIZE, INDUSTRY_GROWTH_MIN,
          LEADERS_PER_INDUSTRY, MAX_STOCKS]).fetchall()
...
def run_backtest(con, rebalance_dates, mktcap_min, use_costs=True, verbose=False,
    ...
    """Run Industry Leader backtest. Returns list of period result dicts."""
    results = []
    ...
    for i in range(len(rebalance_dates) - 1):
        ...
        results.append({
            ...
            "holdings": ",".join(sym for sym, _, _ in clean[:20]),  # truncate long portfolios
        })
...
def run_single(cr, exchanges, universe_name, frequency, use_costs,
    ...
    signal_desc = (f"Industry avg rev growth >= {INDUSTRY_GROWTH_MIN*100:.0f}%, "
                   f"top {LEADERS_PER_INDUSTRY} by revenue/industry, "
                   f"MCap > {mktcap_label} local")
    print_header("INDUSTRY LEADER BACKTEST", universe_name, exchanges, signal_desc)
    ...
    # Start from 2001 (need 2000 data for first growth calculation at July 2001)
    rebalance_dates = generate_rebalance_dates(2001, 2025, frequency,
                                               months=DEFAULT_REBALANCE_MONTHS)
    t0 = time.time()
    con = fetch_data_via_api(cr, exchanges, rebalance_dates, verbose=verbose, offset_days=offset_days)
    if con is None:
    ...
    print(format_metrics(metrics, "Industry Leader", benchmark_name))
    ...
    if annual:
        print(f"\n  {'Year':<8} {'IndustryLeader':>14} {bench_label:>10} {'Excess':>10}")
        print("  " + "-" * 46)
        for ar in annual:
...
def main():
    parser = argparse.ArgumentParser(description="Industry Leader backtest")
    ...
    if args.cloud:
        ...
        result = run_backtest_cloud("industry-leader", args_str=" ".join(cloud_args),
