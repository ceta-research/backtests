"""Generate all Industry Leader charts for blog posts from exchange_comparison.json."""
...
EXCHANGE_LABELS = {
    "NYSE_NASDAQ_AMEX": "Industry Leaders US",
    "BSE_NSE": "Industry Leaders India",
    "XETRA": "Industry Leaders Germany",
    "STO": "Industry Leaders Sweden",
    "TSX": "Industry Leaders Canada",
    "SHZ_SHH": "Industry Leaders China",
    "HKSE": "Industry Leaders HK",
    "JPX": "Industry Leaders Japan",
    "LSE": "Industry Leaders UK",
    "KSC": "Industry Leaders Korea",
    "SIX": "Industry Leaders Switzerland",
    "TAI": "Industry Leaders Taiwan",
    "SET": "Industry Leaders Thailand",
}
...
def get_spy_cumulative(initial=10000):
    """Get SPY cumulative from the US exchange."""
...
def chart_cumulative(exchanges, filename, title, footer_universe):
    ...
    for ex_key in exchanges:
        ...
        ax.annotate(f"${final_k:,.0f}K",
                    ...
                    color=COLORS.get(ex_key, "#95a5a6"))
    ...
    fig.text(0.5, -0.02,
             f"Data: Ceta Research | {footer_universe}, annual rebalance, 2001-2025",
...
def chart_comparison_cagr(filename):
    ...
    # Exclude documented problematic exchanges
    exclude = {"ASX", "SAO", "JNB", "SES"}
    exchanges_with_data = [
    ...
    excess = [v["comparison"]["excess_cagr"] for k, v in exchanges_with_data]
    ...
    ax.set_title("Industry Leaders CAGR by Exchange (2001-2025)", fontsize=14, fontweight="bold", pad=15)
    ax.legend(fontsize=10)
    ax.grid(True, alpha=0.3, axis="x", linestyle="--")

    for i, (cagr, ex_val) in enumerate(zip(cagrs, excess)):
        x_pos = max(cagr, 0) + 0.2
        ex_str = f" ({ex_val:+.1f}%)" if ex_val is not None else ""
        ax.text(x_pos, i, f"{cagr:.1f}%{ex_str}", va="center", fontsize=9, fontweight="bold")

    fig.text(0.5, -0.02,
             "Data: Ceta Research | Top 3 by revenue per growing industry, annual rebalance",
             ha="center", fontsize=8, color="#7f8c8d")
...
# --- Generate all charts ---
print("Generating charts for Industry Leader blogs...")

print("\nUS charts...")
chart_cumulative(
    ["NYSE_NASDAQ_AMEX"], "1_us_cumulative_growth.png",
    "Growth of $10,000: Industry Leaders US vs S&P 500 (2001-2025)",
    "NYSE + NASDAQ + AMEX"
)
chart_annual_bars(
    ["NYSE_NASDAQ_AMEX"], "2_us_annual_returns.png",
    "Industry Leaders US: Year-by-Year Returns (2001-2024)",
    "NYSE + NASDAQ + AMEX"
)
...
chart_cumulative(
    ...
    "Growth of $10,000: Industry Leaders India vs S&P 500 (2001-2025)",
    "BSE + NSE (returns in INR)"
)
chart_annual_bars(
    ["BSE_NSE"], "2_india_annual_returns.png",
    "Industry Leaders India: Year-by-Year Returns (2001-2024)",
    "BSE + NSE (returns in INR)"
)
