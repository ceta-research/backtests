"""Generate all EV/EBITDA Value Screen charts for blog posts from exchange_comparison.json."""
...
EXCHANGE_LABELS = {
    "NYSE_NASDAQ_AMEX": "EV/EBITDA US (NYSE+NASDAQ+AMEX)",
    "NSE": "EV/EBITDA India (NSE)",
    "TSX": "EV/EBITDA Canada (TSX)",
    "STO": "EV/EBITDA Sweden (STO)",
    "XETRA": "EV/EBITDA Germany (XETRA)",
    "MIL": "EV/EBITDA Italy (MIL)",
    "SIX": "EV/EBITDA Switzerland (SIX)",
    "SHZ_SHH": "EV/EBITDA China (SHZ+SHH)",
    "SET": "EV/EBITDA Thailand (SET)",
    "KSC": "EV/EBITDA Korea (KSC)",
    "TAI": "EV/EBITDA Taiwan (TAI)",
    "HKSE": "EV/EBITDA Hong Kong (HKSE)",
    "TLV": "EV/EBITDA Israel (TLV)",
    "JNB": "EV/EBITDA South Africa (JNB)",
    "OSL": "EV/EBITDA Norway (OSL)",
    "SES": "EV/EBITDA Singapore (SES)",
}
...
def chart_comparison_cagr(filename):
    ...
    ax.set_title("EV/EBITDA Value Screen CAGR by Exchange (2000-2025)", fontsize=14, fontweight="bold", pad=15)
    ...
    fig.text(0.5, -0.02,
             "Data: Ceta Research | Same EV/EBITDA screen, annual rebalance, equal weight. Local currency returns.",
             ha="center", fontsize=8, color="#7f8c8d")
...
def chart_comparison_drawdown(filename):
    ...
    names = [EXCHANGE_LABELS.get(k, k) for k, v in exchanges_with_data]
    drawdowns = [abs(v["portfolio"]["max_drawdown"]) for k, v in exchanges_with_data]
    colors = [COLORS.get(k, "#95a5a6") for k, v in exchanges_with_data]

    fig, ax = plt.subplots(figsize=(12, 8))
    bars = ax.barh(range(len(names)), drawdowns, color=colors, alpha=0.85, height=0.6)

    spy_dd = abs(data["NYSE_NASDAQ_AMEX"]["spy"]["max_drawdown"])
    ...
    ax.set_title("EV/EBITDA Value Screen Max Drawdown by Exchange (2000-2025)", fontsize=14, fontweight="bold", pad=15)
    ...
    fig.text(0.5, -0.02,
             "Data: Ceta Research | Same EV/EBITDA screen, annual rebalance, equal weight.",
             ha="center", fontsize=8, color="#7f8c8d")
...
chart_cumulative(
    ...
    "Growth of $10,000: EV/EBITDA US vs S&P 500 (2000-2025)",
    "NYSE + NASDAQ + AMEX"
)
chart_annual_bars(
    ["NYSE_NASDAQ_AMEX"], "us_annual_returns.png",
    "EV/EBITDA US vs S&P 500: Year-by-Year Returns (2000-2025)",
    "NYSE + NASDAQ + AMEX"
)
...
chart_cumulative(
    ...
    f"Growth of $10,000: EV/EBITDA India vs {benchmark_label(data, 'NSE')} (2000-2025)",
    "NSE (returns in INR)"
)
chart_annual_bars(
    ["NSE"], "india_annual_returns.png",
    f"EV/EBITDA India vs {benchmark_label(data, 'NSE')}: Year-by-Year Returns (2000-2025)",
    "NSE (returns in INR)"
)
...
chart_cumulative(
    ...
    f"Growth of $10,000: EV/EBITDA Canada vs {benchmark_label(data, 'TSX')} (2000-2025)",
    "TSX (returns in CAD)"
)
chart_annual_bars(
    ["TSX"], "canada_annual_returns.png",
    f"EV/EBITDA Canada vs {benchmark_label(data, 'TSX')}: Year-by-Year Returns (2000-2025)",
    "TSX (returns in CAD)"
)
...
chart_cumulative(
    ...
    f"Growth of $10,000: EV/EBITDA Sweden vs {benchmark_label(data, 'STO')} (2000-2025)",
    "STO (returns in SEK)"
)
chart_annual_bars(
    ["STO"], "sweden_annual_returns.png",
    f"EV/EBITDA Sweden vs {benchmark_label(data, 'STO')}: Year-by-Year Returns (2000-2025)",
    "STO (returns in SEK)"
)

print("Generating charts for blogs/italy/...")
chart_cumulative(
    ["MIL"], "italy_cumulative_growth.png",
    f"Growth of $10,000: EV/EBITDA Italy vs {benchmark_label(data, 'MIL')} (2000-2025)",
    "MIL (returns in EUR, benchmark in USD)"
)
chart_annual_bars(
    ["MIL"], "italy_annual_returns.png",
    f"EV/EBITDA Italy vs {benchmark_label(data, 'MIL')}: Year-by-Year Returns (2000-2025)",
    "MIL (returns in EUR)"
)
...
chart_cumulative(
    ...
    f"Growth of $10,000: EV/EBITDA Germany vs {benchmark_label(data, 'XETRA')} (2000-2025)",
    "XETRA (returns in EUR)"
)
chart_annual_bars(
    ["XETRA"], "germany_annual_returns.png",
    f"EV/EBITDA Germany vs {benchmark_label(data, 'XETRA')}: Year-by-Year Returns (2000-2025)",
    "XETRA (returns in EUR)"
)
...
print("  us/: 1_us_cumulative_growth.png, 2_us_annual_returns.png")
print("  india/: 1_india_cumulative_growth.png, 2_india_annual_returns.png")
print("  canada/: 1_canada_cumulative_growth.png, 2_canada_annual_returns.png")
print("  sweden/: 1_sweden_cumulative_growth.png, 2_sweden_annual_returns.png")
print("  italy/: 1_italy_cumulative_growth.png, 2_italy_annual_returns.png")
print("  germany/: 1_germany_cumulative_growth.png, 2_germany_annual_returns.png")
print("  comparison/: 1_comparison_cagr.png, 2_comparison_drawdown.png")
