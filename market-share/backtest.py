#!/usr/bin/env python3
"""
Market Share Gain Backtest

Annual rebalancing (July), equal weight, top 30 by sector-relative revenue growth.
...
Portfolio: Top 30 by sector-relative revenue growth, equal weight. Cash if < 10 qualify.
Rebalancing: Annual (July), 2000-2025.

Academic reference: Piotroski & So (2012), Jegadeesh & Livnat (2006), and the
broader fundamental momentum literature support revenue growth as a market
share proxy.

Usage:
    # Backtest US stocks (default)
    python3 market-share/backtest.py
    ...
    python3 market-share/backtest.py --no-costs
...
def fetch_data_via_api(client, exchanges, rebalance_dates, verbose=False,
    ...
    Populates tables:
        profile_cache(symbol VARCHAR, sector VARCHAR)
    ...
    # 1. Universe + sector (needed for sector-median computation)
    print("  Fetching exchange membership and sector data...")
    ...
    sym_values = ",".join(f"('{r['symbol']}', '{(r.get('sector') or '').replace(chr(39), chr(39)+chr(39))}')"
                          for r in profiles)
    con.execute(f"CREATE TABLE profile_cache(symbol VARCHAR, sector VARCHAR); INSERT INTO profile_cache VALUES {sym_values}")
    ...
    # Build benchmark symbols list (SPY + local benchmark)
...
def screen_stocks(con, target_date, mktcap_min):
    """Screen for market-share gainers (sector-relative revenue growth).

    Computes YoY revenue growth from two consecutive FY income statements.
    Computes sector medians, filters for excess_growth >= EXCESS_GROWTH_MIN.
    Returns list of (symbol, market_cap) tuples sorted by excess_growth DESC.
    """
    ...
    rows = con.execute("""
        ...
        rev_p AS (
            SELECT symbol, revenue AS prior_revenue, filing_epoch AS prior_filing_epoch,
                ROW_NUMBER() OVER (PARTITION BY symbol ORDER BY filing_epoch DESC) AS rn
            FROM income_cache
            WHERE filing_epoch <= ?
        ),
        ...
        growth AS (
            SELECT rc.symbol, pc.sector,
                (rc.revenue - rp.prior_revenue) / rp.prior_revenue AS rev_growth,
                km.returnOnEquity, km.marketCap, fr.operatingProfitMargin
            FROM rev_c rc
            JOIN rev_p rp ON rc.symbol = rp.symbol AND rp.rn = 1
            JOIN km ON rc.symbol = km.symbol AND km.rn = 1
            JOIN fr ON rc.symbol = fr.symbol AND fr.rn = 1
            JOIN profile_cache pc ON rc.symbol = pc.symbol
            WHERE rc.rn = 1
              AND rp.prior_revenue > 0
              AND rc.filing_epoch != rp.prior_filing_epoch
              AND km.returnOnEquity > ?
              AND fr.operatingProfitMargin > ?
              AND km.marketCap > ?
              AND pc.sector IS NOT NULL
              AND pc.sector != ''
        ),
        sector_stats AS (
            SELECT sector,
        ...
        SELECT g.symbol, g.marketCap,
            (g.rev_growth - ss.median_growth) AS excess_growth
        FROM growth g
        JOIN sector_stats ss ON g.sector = ss.sector
        WHERE (g.rev_growth - ss.median_growth) >= ?
        ORDER BY excess_growth DESC
        LIMIT ?
    """, [cutoff_epoch, prior_cutoff_epoch, cutoff_epoch, cutoff_epoch,
          ROE_MIN, OPM_MIN, mktcap_min, EXCESS_GROWTH_MIN, MAX_STOCKS]).fetchall()
...
def run_backtest(con, rebalance_dates, mktcap_min, use_costs=True, verbose=False,
    ...
    """Run Market Share backtest. Returns list of period result dicts."""
    results = []
...
def run_single(cr, exchanges, universe_name, frequency, use_costs,
    ...
    signal_desc = (f"Excess Rev Growth >= {EXCESS_GROWTH_MIN*100:.0f}pp, "
    ...
    print_header("MARKET SHARE GAIN BACKTEST", universe_name, exchanges, signal_desc)
    ...
    print(format_metrics(metrics, "Market Share", benchmark_name))
    ...
    if annual:
        ...
        print(f"\n  {'Year':<8} {'MarketShare':>12} {bench_col:>10} {'Excess':>10}")
        print("  " + "-" * 42)
        for ar in annual:
...
def main():
    parser = argparse.ArgumentParser(description="Market Share Gain backtest")
