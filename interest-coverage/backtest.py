#!/usr/bin/env python3
"""
Interest Coverage Screen Backtest

Quarterly rebalancing, equal weight, top 30 by highest interest coverage.
Fetches data via configurable provider, caches in DuckDB, runs locally.

Signal: Coverage > 5, D/E 0-1.5, ROE > 8%, Market Cap > $1B
Portfolio: Top 30 by highest coverage, equal weight. Cash if < 10 qualify.
...
def fetch_data_via_api(client, exchanges, rebalance_dates, verbose=False):
    ...
    Populates tables:
        ...
        ratios_cache(symbol, interestCoverageRatio, debtToEquityRatio, filing_epoch, period)
    ...
    queries = [
        ...
        ("ratios_cache", f"""
            SELECT symbol, interestCoverageRatio, debtToEquityRatio, dateEpoch as filing_epoch, period
            FROM financial_ratios
            WHERE period = 'FY' AND interestCoverageRatio IS NOT NULL AND {sym_filter_sql}
        """, "financial ratios (coverage, D/E)"),
    ]
...
def run_backtest(con, rebalance_dates, mktcap_min, use_costs=True, verbose=False,
    ...
    for i in range(len(rebalance_dates) - 1):
        ...
        # not changed here.
        #
        # `symbol_data` above is built with the EXIT price already filtered out,
        # so entry_buyable() over it would still be exit-conditioned. Count from
        # the screened symbols and the entry-price map instead.
...
def main():
    parser = argparse.ArgumentParser(description="Interest Coverage Screen backtest")
    ...
    if args.cloud:
        ...
        result = run_backtest_cloud("interest-coverage", args_str=" ".join(cloud_args),
    ...
    if annual:
        print(f"\n  {'Year':<8} {'Coverage':>10} {benchmark_name:>10} {'Excess':>10}")
        print("  " + "-" * 40)
        for ar in annual:
