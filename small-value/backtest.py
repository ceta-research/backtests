#!/usr/bin/env python3
"""
Small-Cap Value Backtest

Annual rebalancing (July), equal weight, top 30 by lowest P/B.
Fetches data via Ceta Research API, caches in DuckDB, runs locally.

Signal: Small-cap range (5%-200% of exchange standard threshold),
        P/B ratio 0-1.5, ROE > 5%, D/E < 2.5
        Ranked by P/B ascending (cheapest book value first).

Portfolio: Top 30 by lowest P/B, equal weight. Cash if < 10 qualify.
Rebalancing: Annual (July), 2000-2025.

Academic references:
  - Fama & French (1992) "The Cross-Section of Expected Stock Returns",
    Journal of Finance 47(2), 427-465. Size and book-to-market as systematic factors.
  - Fama & French (1993) "Common Risk Factors in the Returns on Stocks and Bonds",
    Journal of Financial Economics 33(1), 3-56. SMB + HML three-factor model.
  - Lakonishok, Shleifer & Vishny (1994) "Contrarian Investment, Extrapolation,
    and Risk", Journal of Finance 49(5), 1541-1578. Value premium in small stocks.
...
Usage:
    ...
    python3 small-value/backtest.py --no-costs
...
# --- Signal parameters ---
PB_MIN = 0.0               # Must have positive book value (no negative equity traps)
PB_MAX = 1.5               # Value territory: P/B < 1.5
ROE_MIN = 0.05             # Return on equity > 5% (avoid deep value traps)
DE_MAX = 2.5               # Debt-to-equity < 2.5 (not over-leveraged)
MAX_STOCKS = 30            # Top 30 by lowest P/B, equal weight
...
MIN_ENTRY_PRICE = 0.50     # Skip stocks with entry price < $0.50 (micro-cap artifact guard)
...
def fetch_data_via_api(client, exchanges, rebalance_dates, verbose=False):
    ...
    Populates tables:
        ...
        ratios_cache(symbol, priceToBookRatio, debtToEquityRatio, filing_epoch, period)
    ...
    Note: priceToBookRatio and debtToEquityRatio live in financial_ratios.
          returnOnEquity and marketCap live in key_metrics.
    """
    if exchanges:
    ...
    # 2. P/B and D/E from financial_ratios (FY annual data)
    print("  Fetching P/B and D/E ratios (financial_ratios)...")
    ratios_sql = f"""
        SELECT symbol, priceToBookRatio, debtToEquityRatio, dateEpoch as filing_epoch, period
        FROM financial_ratios
        WHERE period = 'FY'
          AND priceToBookRatio IS NOT NULL
          AND priceToBookRatio > 0
          AND {sym_filter_sql}
    """
    ...
    # 3. ROE and market cap from key_metrics (FY annual data)
    print("  Fetching ROE and market cap (key_metrics)...")
    metrics_sql = f"""
...
def screen_stocks(con, target_date, small_cap_min, small_cap_max):
    """Screen for small-cap value stocks.

    Returns list of (symbol, market_cap) tuples sorted by P/B ascending.
    Uses 45-day lag for point-in-time data integrity (annual filings).

    Filters:
        - P/B: 0 < P/B < 1.5 (value territory, positive book value)
        - ROE: > 5% (profitability guard, avoids value traps)
        - D/E: < 2.5 (leverage guard; NULL D/E accepted for unlevered firms)
        - Market cap: within small-cap bounds for the exchange
    """
    ...
    rows = con.execute("""
        WITH r AS (
            SELECT symbol, priceToBookRatio, debtToEquityRatio, filing_epoch,
...
def run_single(cr, exchanges, universe_name, frequency, use_costs,
    ...
    signal_desc = (f"MCap {min_label}–{max_label} local, P/B 0–{PB_MAX}, "
                   f"ROE>{ROE_MIN*100:.0f}%, D/E<{DE_MAX}, top {MAX_STOCKS} by P/B")
    print_header("SMALL-CAP VALUE BACKTEST", universe_name, exchanges, signal_desc)
    ...
    if annual:
        print(f"\n  {'Year':<8} {'SmallValue':>12} {benchmark_name:>10} {'Excess':>10}")
        print("  " + "-" * 42)
        for ar in annual:
...
def main():
    ...
    if args.cloud:
        ...
        result = run_backtest_cloud("small-value", args_str=" ".join(cloud_args),
