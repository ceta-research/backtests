...
ASSET_GROWTH_MIN = -0.20   # Exclude shrinking companies (> -20%)
...
def fetch_data_via_api(client, exchanges, rebalance_dates, verbose=False):
    ...
    Populates tables:
        ...
        balance_cache(symbol, totalAssets, filing_epoch, period)
    ...
    # 2. Balance sheet (need two consecutive FY filings for YoY growth)
    queries = [
        ("balance_cache", f"""
            SELECT symbol, totalAssets, dateEpoch as filing_epoch, period
            FROM balance_sheet
            WHERE period = 'FY'
    ...
    # 4. Prices (only at rebalance dates, with next-day offset window)
    print("  Fetching prices...")
    date_conditions = []
    for d in rebalance_dates:
        end = d + timedelta(days=15)  # Extra days for offset_days=1 + weekend buffer
...
def screen_stocks(con, target_date, mktcap_min):
    ...
    rows = con.execute("""
        ...
        SELECT bc.symbol, km.marketCap,
            (bc.totalAssets - bp.totalAssets_prior) / bp.totalAssets_prior AS asset_growth
        FROM bs_current bc
        JOIN bs_prior bp ON bc.symbol = bp.symbol AND bp.rn = 1
        JOIN km ON bc.symbol = km.symbol AND km.rn = 1
        JOIN fr ON bc.symbol = fr.symbol AND fr.rn = 1
        WHERE bc.rn = 1
          AND bp.totalAssets_prior > 0
          AND bc.filing_epoch != bp.filing_epoch
          AND (bc.totalAssets - bp.totalAssets_prior) / bp.totalAssets_prior < ?
          AND (bc.totalAssets - bp.totalAssets_prior) / bp.totalAssets_prior > ?
...
def run_single(cr, exchanges, universe_name, frequency, use_costs,
    ...
    print(format_metrics(metrics, "Asset Growth", benchmark_name))
    ...
    if annual:
        ...
        print(f"\n  {'Year':<8} {'AssetGrowth':>12} {bench_label:>10} {'Excess':>10}")
        print("  " + "-" * 42)
        for ar in annual:
...
def main():
    parser = argparse.ArgumentParser(description="Asset Growth Anomaly backtest")
    ...
    if args.cloud:
        ...
        result = run_backtest_cloud("asset-growth", args_str=" ".join(cloud_args),
