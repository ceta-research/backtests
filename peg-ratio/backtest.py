...
a Reasonable Price. PEG < 1 means you're paying less than the growth rate warrants.
...
PEG_MIN = 0.0              # Must be positive (positive earnings + earnings growth)
PE_MIN = 8                 # Exclude distressed / deeply cyclical
...
def fetch_data_via_api(client, exchanges, rebalance_dates, verbose=False):
    ...
    # 2. Key metrics (ROE, market cap)
    # Only fetch rows with positive PEG ratio to reduce data volume
    queries = [
        ("metrics_cache", f"""
...
def run_single(cr, exchanges, universe_name, frequency, use_costs,
    ...
    print(format_metrics(metrics, "PEG Ratio", benchmark_name))
    ...
    if annual:
        print(f"\n  {'Year':<8} {'PEG Ratio':>12} {benchmark_name:>10} {'Excess':>10}")
        print("  " + "-" * 42)
        for ar in annual:
...
def main():
    parser = argparse.ArgumentParser(description="PEG Ratio (GARP) backtest")
    ...
    if args.cloud:
        ...
        result = run_backtest_cloud("peg-ratio", args_str=" ".join(cloud_args),
