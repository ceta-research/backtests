...
def fetch_data_via_api(client, exchanges, rebalance_dates, verbose=False):
    ...
    # Need prices at entry AND exit dates (+ benchmarks)
    # Also fetch next-day prices for MOC execution (offset_days=1)
...
def run_backtest(con, rebalance_dates, mktcap_min, use_costs=True, verbose=False,
    ...
    for i in range(len(rebalance_dates) - 1):
        ...
        # not changed here.
        #
        # `raw_returns` above is built with the EXIT price already filtered out,
        # so entry_buyable() over it would still be exit-conditioned. Count from
        # the screened symbols and the entry-price map instead. 0.50 matches this
        # topic's own filter_returns call.
        buyable = entry_buyable_prices(symbols, entry_prices, min_entry_price=0.50)
...
def main():
    ...
    if annual:
        ...
        print(f"\n  {'Year':<8} {'Net-Net':>10} {bench_col:>10} {'Excess':>10}")
        print("  " + "-" * 40)
        for ar in annual:
